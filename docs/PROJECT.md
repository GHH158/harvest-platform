...
| 大语言模型 | **Qwen 文本模型优先，DeepSeek 备用** | 阿里云百炼 / DeepSeek | 百炼免费额度优先；耗尽后切换备用模型 |
...
**可选优化**(第二版再考虑):OSS 生命周期自动清理。HLS 已是 P5 正式范围,不再属于可选项。
...
④ Worker:ffmpeg 转码为 720p @1.5Mbps HLS(6 秒分片),同时生成纯音频 HLS 和 ASR 临时完整音轨
⑤ Worker:HLS 清单/分片与临时音轨上传 OSS → 临时音轨调用 Fun-ASR → 日语字幕 + 时间轴
⑥ Worker:调用 LLM 翻译 → 中文字幕
⑦ Worker:保留视频/音频 HLS,删除 OSS 上的 ASR 临时音轨
...
2. 到准备实际使用服务的那一周，再统一注册、配置 Key，并依次完成 P1 → P2 → P3 → P4 → P5 → P6 的云链路实调与闸 B 证据。某阶段的云链路未通过，不得把该阶段或其依赖结论标为通过。
