...
### 15.4 切分与转码合成一步,因此切点精确到帧

`ffmpeg -c copy` 能无损秒切,但**只能切在关键帧上**。典型关键帧间隔 2–10 秒,于是「在 5:04.3 打的点」会被挪到最近的关键帧,可能刚好切掉半句——精心打的点白打。

避开它不需要付额外代价,因为 **§3.4 已经写着「只有本地上传、编码不兼容或直接封装失败时才转码」——手机上传的视频本来就要转码一次**。既然无论如何都要重新编码,就**把切分和转码合成一步**:直接从原片的 `[start, end)` 转出每一节的 HLS。
