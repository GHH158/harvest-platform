...
2026-08-07 修订:原文写「不做课本数字化」,现放宽为**不搬运课本正文,但可以有系统性的语法骨架**(§12)。理由见 §1.4 的对应条目。
...
- 不录入课本正文。**但允许维护一份语法点目录并按需生成讲解**(§12):存的是「有哪些点、你处于什么状态」,讲解正文由 §5.8 的教学内核实时生成,不预置成套教材文本。
  修订理由:原判断认为基础靠课本、软件只做课本做不了的事。实际使用暴露了一个课本也解决不了的缺口——使用者能在陪读和纠错里逐点听懂,但这些解释是**一次性且离散的**,遇到过什么、还差什么,没有任何地方能看到。课本给的是别人的顺序,这里要的是**按自己实际撞见过的内容组织起来的骨架**,这一点课本做不到。范围仍受限:不抄写课本例句与讲解,不做课程进度条,不做打卡。
...
- 遵守「结构自显」:不再新增仅为强调而存在的左侧竖线、彩色边框或标签条;优先用排版、留白、底色与位置表达层级。现有 Markdown 标题强调条列入后续 UI 专项复核,不在文档审计中直接改代码
...
**现象**(2026-08-09 架构复核):「不抢跑」的纪律在**功能层面**守住了——拒绝向量库、拒绝扩建角色阵容、拒绝没有第三个消费者的通用 `LearnerState` 表,这些都拒得对。但在**契约层面**没有守住:现在是 23 张表、448 行基线 schema、14 个触发器、§5 十四个小节、§13.9 七个里程碑九道闸,每个里程碑都在新增需要长期维护的契约、版本号和文档段落。
...
**当前判断**:不砍设计,契约本身质量是够的。改的是排期方式——**把「真实使用」当成与写代码同等的一等资源**。具体一条:M3 开始编码前,先按 §9 的建议正常使用这个 App 一段时间,让事件表里长出真实数据,M3/M4 的验收才有素材可用。这是 §9「建工具本身太有成就感,会伪装成学习」在工程排期上的具体落点,不是一句提醒。
...
按以下顺序逐个接入,每个入口都先定义「什么是证据,什么不是」:

1. 陪读中的明确提问与重新表达;
2. 全局聊天中的纠错、话题与真实表达;
3. 存词与复习回答;
4. 跟读的漏词、发音和节奏结果;
5. 阅读与视频中的目标句、主动提问与理解语境;
6. 拍照材料与实时语音中的表达与对话策略。

通过条件:任一入口产生的高价值证据都能在另一个合适入口被按需召回,且不相关的历史不会因「语义看起来相似」而干扰当前任务。

#### M5 — 关系记忆与 App 内主动建议

目标:让角色不只记得知识弱项,也记得共同经历和未完成的对话。

- 每轮完成后用两阶段守门员判断是否有值得保留的关系记忆;
- 记忆条目有事件依据、关系角色、置信度、状态和冲突/归档语义;
- 角色可拥有第一人称的内部关系/情感日记,原文可以不逐条展示,但必须明确其为 AI 的主观表达状态,不能包装成真实感受或用户事实;用户至少能查看类别与总体状态、关闭提取并删除该类数据;
- 情感日记与执行/教学管道物理或逻辑隔离:它可以改变措辞温度、角色是否发出一次克制邀请,不得改变教学事实、纠错标准、帮助质量、功能权限或是否响应用户;
- 先只做 App 内克制建议,例如延续上次未完成的表达,不立即上系统推送;
- 用户可查看系统总体记住了什么、删除来源证据、关闭某类记忆并调低主动程度;
- 连续忽略后自动降低同类建议频率,不用关系和角色施加打卡压力。

通过条件:主动建议能指向真实未完成线索;误记可被完整删除;用户拒绝或忽略会真正改变后续行为。

#### M6 — 沉浸圆桌与动态学习场景

目标:把知识库中的真实线索变成有关系、有意图的新语境,而不是换皮练习题。

- 几位稳定角色围绕自然话题用日语交流,用户可旁听、加入或单独回应某个人;
- 动态场景从声明式结构开始,包含场景、参与者、开场、可选帮助、表达机会与完成信号;
- 同一场景的文案、角色位置、时间、已发生事件与后续邀请必须由同一份结构化场景状态派生,不得让多个生成器各自编造一套世界状态;
- 最近知识只在不破坏对话真实性时轻量放入,不为了覆盖率强行塞词或语法;
- 用户的新表达回到事件层,不把角色创作的剧情当成用户事实;
- 任何虚构经历、角色生活或沉浸剧情都要有不打断体验但始终可查的 AI 虚构声明;不建设以填满时间线、增加消费时长为目标的通用朋友圈,动态内容必须服务于再次理解或真实日语输出;
- 只在声明式动态体验被实测证明有长期价值后,再设计沙箱化可执行页面的权限、安全检查与撤销机制。

通过条件:用户会主动加入或继续这些场景,且其中有可观察的真实日语输出;不用「观看了多少 AI 生成内容」作为成功指标。

#### M7 — 语义召回与规模化

目标:只在真实历史规模证明结构化关系不足以找回有用经历时,引入向量检索和更完整的记忆压缩。

- 先建立可人工复核的真实召回样本,记录应召回与不应召回的历史;
- 固定召回顺序:明确关系 → 时间/状态筛选 → 语义候选 → 沿结构化关系扩展;
- 向量记忆在完整互动结束后写入,防止把模型刚生成的话反复召回成「历史事实」;
- 检索不可用时降级到精确关系与时间检索,不影响核心学习入口;
- 根据命中率、误召回、漏召回、延迟与成本决定是否继续扩展,不以「已经接入向量库」作为成功。

通过条件:在真实样本上比纯结构化召回有明确增益,误召回可解释且可删除,向量索引可从原始证据完整重建。

#### 共同验收闸

每个里程碑除各自通过条件外,还必须同时过以下九道闸:

1. **数据闸**:证据可追溯,投影可重建,删除能清理关联,同一事件重放幂等。
2. **准确性闸**:不把角色偏好写成语法事实,不把 AI 输出写成用户证据,不为制造热闹编造分歧。
3. **体验闸**:表面交互比内部机制更简单,不向用户暴露模型、调度、记忆分层与数据表管理负担。
4. **学习闸**:功能促成新的理解、提问或真实日语输出,不以消费更多 AI 内容和更长使用时间替代学习效果。
5. **退化闸**:记忆、圆桌、角色或语义检索不可用时,阅读、播放、查词、陪读和聊天主路径仍可继续。
6. **接口闸**:跨入口共享的契约有稳定标识、版本和兼容规则;UI 事件、领域事实与后台决策不得共用一个含混的「event」结构。
7. **可观测与隐私闸**:关键决策记录调用目的、原因、版本、证据引用、耗时和失败阶段;默认不复制完整私密原文,详细日志只能显式开启。
8. **权限与撤销闸**:任何 AI 主动写入长期记忆、改变配置、生成可执行页面或产生外部副作用的能力都要有最小权限、用户门控与操作前快照;不确定时拒绝或降级,不能自动放行。
9. **成本闸**:后台模型调用、多角色数量、重试与主动生成都有明确上限和调用来源;用户能看到长任务正在运行并可中止,不得用隐藏消耗制造活跃假象。

### 13.10 Alice 方法论审计与 Harvest 采用矩阵（2026-08-08）

#### 审计范围与裁决方式

本次审计覆盖 Alice 官网方法论目录的全部内容:序章、第一至十五章工程方法、产品观一至五、附录决策清单,以及七篇创作者故事。逐章核对的是问题定义、实际踩坑、设计决策、适用边界与可迁移结论,不是只看首页功能介绍。权威入口:

- `https://alice.miyang.cn/methodology/`
- `https://github.com/itshen/Alice_methodology`

Alice 是通用桌面 Agent 的方法论,Harvest 是个人日语学习系统。审计不以「复制多少功能」计分,而用以下四种裁决:

1. **直接采用**:与领域无关的工程底线,从当前里程碑起持续约束所有实现。
2. **转译采用**:保留底层原理,换成学习领域的对象、交互和成功指标。
3. **证据后采用**:方向有价值,但只有真实使用证明现有方案不足时才引入基础设施。
4. **明确排除**:与学习目标、诚实边界或单用户范围冲突,即使 Alice 有实现也不照搬。

#### 工程方法采用矩阵

| Alice 章节 / 原则 | Harvest 裁决 | 落点与边界 |
|---|---|---|
| 序章、第一章、十五章、附录:接口长寿、状态优先、结构隔离、可丢弃、可观测 | **直接采用** | §13.2、§13.7、§13.9 九道闸。先回答状态住哪里、谁读写、如何重放和降级,再编码;稳定契约优先于实现细节 |
| 第二章:系统地图、分层架构、核心逻辑与 UI 分离 | **转译采用** | 不复制 Electron 三进程。Harvest 保留 iOS / FastAPI / worker / PostgreSQL 边界;领域逻辑输出纯数据,UI 不自行推断学习事实 |
| 第三章:Agent Loop 与事件流出口 | **证据后采用** | 普通查词、陪读和结构化纠错继续用简单请求/响应;只有圆桌、长时动态场景或可中止后台任务确实需要持续进度时才引入 UI 事件流。领域 `LearningEvent` 不等于 UI 流式事件 |
| 第四章:声明式工具、先读后写、并发元数据、工具/服务隔离 | **转译采用** | 当前学习推断、投影、缓存失效属于内部服务,不暴露成 AI 工具。未来角色或动态页面若获得工具能力,必须声明只读、并发、权限、输出上限与适用/排除场景 |
| 第五章与记忆故事:按生命周期分层、对话结束后写入、防自我强化、守门员、删除即还原 | **直接采用逻辑,基础设施按证据演进** | §13.3、M1、M5、M7。先用 SQL 精确关系;向量只在真实召回样本证明必要后加入;五层是逻辑边界,不是同时部署五套存储 |
| 第六章与多 Agent 故事:父子/协调/Swarm 选型、上下文隔离、角色白名单、收敛 | **转译采用** | M2/M3 第一版只用中心化主持 + 少量独立角色;不共享消息数组、不嵌套主持、不让角色直接写学习者事实;没有多角度价值时保持单 Agent |
| 第七章:最小权限、默认保守、原因可审计、失败安全 | **直接采用到有副作用的能力** | 当前 AI 讲解无外部写操作;长期记忆写入、主动建议、配置改变、动态页面和外部渠道出现前必须先定义权限与拒绝/确认路径 |
...
| Alice 产品观 | Harvest 裁决 | Harvest 的转译 |
|---|---|---|
| 活人感来自设定集、连续记忆、统一状态机与边界 | **转译采用** | M2 起为主持人与角色建立可版本化设定集;角色行为从同一关系/场景状态派生,不靠每轮随机扮演 |
| 人格核心区与可变关系区分离 | **直接采用** | 教学准确性、诚实性和角色核心身份受保护;措辞偏好、熟悉度和共同经历可以渐进变化 |
| 情感日记与隐藏主观评价 | **受约束采用** | M5 可有角色第一人称关系日记,原文不必逐条展示;它必须明确是 AI 主观表达状态,用户能关闭和删除该类数据,不能成为秘密能力评分、道德标签或惩罚依据 |
| 情绪影响表达,不影响执行 | **直接采用** | 关系状态可改变措辞温度和邀请时机,不能改变日语事实、纠错标准、帮助质量、功能权限或是否响应 |
| 多 Agent 都有完整人格 | **转译采用** | 只给确有稳定教学差异的角色建立人设;角色数量由辨识度、学习价值和成本决定,不追求 Alice 的人数 |
| DayScript、预规划生活与 RPG 状态机 | **只采用一致性原则** | 动态学习场景要有统一的结构化时间/角色/事件状态;不在学习价值尚未验证时模拟一整套离线生活 |
| 朋友圈与主动社交 | **不直接采用功能形态** | 不建设以填满时间线、制造依赖或增加消费为目标的通用动态 Feed。未来角色动态只作为学习场景的副产品,必须产生理解、提问或日语输出 |
| 虚构生活可以增强沉浸 | **受约束采用** | 虚构角色和剧情始终有可查的 AI 虚构声明;剧情可以帮助练习,不能伪装成真实经历、真实来源或用户记忆 |
| UI:结构自显、时间连续、颜色有语义、信息分层 | **直接采用原则,不复制 Alice 品牌皮肤** | 保留 Harvest 的暖色 DesignTokens 与 iOS 语言;不新增装饰性左竖线、强边框或无语义颜色,状态变化有连续反馈,普通界面只显示用户当前需要的信息 |
| UI:所有 AI 输出必须流式即时 | **按协议转译** | 自由文本长任务可评估流式;JSON 纠错和需要完整校验的教学输出继续原子提交,但客户端必须立即显示本地消息、进度和可恢复失败,不能让等待像失效 |
| 写作:真实、具体、克制、基于历史、宁缺毋滥 | **直接采用原则** | 用户文案与角色表达不编造来源或经历,减少 AI 套话和装饰。Alice 的逐字句式禁忌属于其个人品牌,不机械复制为 Harvest 全局语法规则 |
| 金钱、钱包、消费与虚拟资产制造脆弱感 | **明确排除当前路线** | 与日语学习目标无关,容易把关系变成消费和养成;不进入 M1–M7 |
| 数值化情绪、好感度和可刷状态 | **明确排除** | 与 §1.4 一致:不展示好感值、掌握率、连续天数或可优化的关系数值,不把自然学习变成策略游戏 |
...
- 不声称角色是真人,不把 AI 生成的情感、生活、读书或旅行包装成现实事实;
- 不让情感或关系状态降低服务和教学质量,不拉黑学习内容,不通过冷淡、愧疚或依赖感施压;
- 不用朋友圈、信件、推送频率或虚拟消费制造留存,成功指标仍是真实理解与日语输出;
- 不因 Alice 使用五层记忆、11 个角色、MCP 或自动页面就预先建设同等规模;
- 不把后台 LLM 调用和多 Agent 成本藏起来,不允许无法中止的长时自动任务;
- 不把「快速发版」理解为跳过测试、文档、权限、安全与回滚。

#### 所有后续功能的编码前置清单

每个 M1–M7 子功能进入编码前,除 §13.9 验收闸外,必须在对应设计段落回答以下问题:

1. 涉及哪些事实、投影、缓存和短期运行状态?生命周期分别是什么?
2. 谁能读、谁能写?AI 输出是否可能被误当成用户事实?
3. 跨入口接口的稳定标识、版本、幂等键和兼容策略是什么?
4. 哪些步骤只读可并行,哪些写入必须串行或放进事务?自动触发是否有递归守卫?
5. 这是用户有意义的动作,还是 AI 不应感知的内部服务?
6. 增强组件失败时如何降质?核心阅读、聊天和播放是否仍可用?
7. 是否产生长期写入、外部副作用或可执行内容?确认、最小权限、快照和撤销在哪里?
8. 如何记录调用目的、原因、证据、版本、耗时、成本和失败阶段?默认记录是否泄露私密原文?
9. 用户表面上要多做几步?能否通过结构与默认值把复杂性留在系统内部?
10. 它最终促成哪一次新的理解、提问或真实日语输出?如果只能增加 AI 内容消费,就不进入开发。

**方法论审计、M1 与 M2-A 安全预览已完成;M2 首轮真实调用因角色同质、trace 缺失与盲评映射泄露而不通过。当前代码只进入 `role-perspective-v2` / `m2-blind-v2` 复验准备,没有再次外部调用授权就不得启动新一轮 30 次测试;复验通过前不得进入 M3 圆桌,也不得扩建角色阵容、向量设施或主动推送。** M3–M7 继续只保留产品方向与验收边界,按真实使用证据逐步开启。
