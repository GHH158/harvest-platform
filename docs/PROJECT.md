...
### 0.4 只记决策,不记验证过程(2026-08-12 起)

**只对之后新写的内容生效,§18 及更早的既有内容不回头精简。**

每条决策写清楚**决定了什么、为什么这么决定**就够了;不要写"跑了什么命令、建了几个临时数据库、探测了几轮、清理了哪些测试数据"这类操作记录——那是过程,不是决策,而且从代码本身就能看出来实现是怎么做的。

**例外:当验证过程本身推翻了一个不直观的假设、或产出了单靠读代码得不到的关键数据时,这个数据要保留**,因为决策的说服力就建立在这条数据上,删掉数据留结论,以后没法判断这个决策当初是不是想当然。判断标准:去掉这段话之后,读者会不会问"凭什么这么说"——会问,就是关键数据,留;不会问,就是操作记录,删。

例:"迁移在真实库的克隆上连跑两次验证幂等"是操作记录,可以不写;"12/13 条 `summary_zh` 实际是日语,与字段名和提示词都相反"是那条改名决策成立的全部理由,必须留。
...
- 减少界面"装饰性元素"(图标堆砌、渐变背景、强分割线),让文字和朗读内容本身是唯一的主角。**2026-08-12 起,§19 记了一个有意的例外**:引入一套线条插画系统,长期目标是贯穿整个应用。这不是推翻本条——插画本身遵循「单一克制焦点」而非堆砌,而是承认「装饰」与「有设计感的视觉元素」不是同一件事。具体先落地到哪些页面,§19 尚未逐一敲定。
...
> **2026-08-11 起,阅读/观看页的即时问答入口已被 §16 取代,本节保留作为历史记录(§0.2 不删除,只标记)。** 独立提问入口 `AskView`(§5.16)不受影响,继续使用本节描述的共同教学内核与陪读专用层。
...
> **2026-08-11 起,本节描述的阅读器即时提问入口已被 §16 取代,本节保留作为历史记录。** 四个角度(lens)本身作为一套系统继续保留、供 §5.16 的独立提问入口 `AskView` 使用,只是不再从阅读正文的点词/点句触发。
...
|---|---|---|---|
| `meaning` | 啥意思？ | 这个词/这句在此处的意思与用法 | 无(实测最常见的真实需求) |
| `naturalness` | 怪不怪 | 听感、语体、说话人意图 | 葵 |
| `structure` | 拆开看看 | 语法关系、接续、信息重心 | 圭 |
| `chinese` | 跟中文比 | 中日差异、同形词、迁移风险 | 林 |

**标签文案 2026-08-10 重写**(原为 意思 / 自然吗 / 结构 / 和中文)。使用者实际用过之后的评价是「这都是些啥」——那四个词是**分类名**,不是人会说的话。改为口语化的说法,只有 `label_zh` 变,`question_zh` 与 `focus_zh` 一字未动,因此 `LENS_PROMPT_VERSION` 不升:模型看到的东西完全没变。

**但 §11.9 的约束没有放松,而且它两次决定了最终文案**:标签必须一眼说清点下去会得到什么,否则你会退回只点第一个。
1. 第一版写成「这么说怪吗 / 跟中文差在哪」,四个胶囊排不下一行,横向滚动把最后一个**切成「跟中文差在」**——读起来像错字,而且正好把「点它得到什么」藏掉了。
2. 改成 2×2 两行解决了可读性,但在三分之二高的弹层里吃掉一行高度。
3. 最终按使用者的第二个提议**缩短文案**(19 字 → 15 字),一行排下且四个完整。横向滚动作为兜底保留——放大系统字号时退化成可滑动,而不是被切掉。
...
> **2026-08-12 起,本节的「独立提问入口」(`AskView` / `POST /ask`)已被 §17 取代并退场,本节保留作为历史记录(§0.2 不删除,只标记)。** 同一节里关于首页结构本身的部分(去掉底部导航、只显示事实计数)继续有效;§17 只把「聊天」提到了头条位,并未推翻这些结论。
...
#### 弹层只显示当前这一句(2026-08-10)

**现象**(使用者实际用过之后提的第一条):点开陪读就刷出大量内容,而那些内容和你刚点的那一句无关。

**根因**:弹层打开时加载**整篇材料**的全部历史(上限 40 条)。而模型那边只取最近 12 条——所以那一屏既不是使用者要的,也不是模型需要的。

**判断:记住和显示是两件事,而它们此前被绑在一起了。**

- **记录必须留。** §12 的语法证据挂在 `companion_message.id` 上(`companion_grammar_evidence.message_id`),§5.11 的 `companion_question` 事件也挂在它上面,§4.3 的删除触发器保证「来源行消失 → 事件消失」。**删聊天记录会连带抹掉语法骨架的来源。**事件是事实(§13.2)。
- **显示完全不必。** §5.17 之后弹层是「针对当前这一句」的,每次提问的对象都是不同的句子。

**改为**:`GET /companion/{id}?segment_id=` 按句过滤,弹层默认只显示当前句的问答;**同一句以前问过的仍然显示**——那是真上下文(先点「拆开看看」再点「怪不怪」是同一场对话);整篇的历史收进一个「看这篇以前问过的」入口,默认不加载,点开后不再收起。

**顺带让本节开头那条判据真正成立**:上限 40 只是把代价**封顶**,并没有让它与历史长度**无关**;按句过滤之后才是。

**一条对 §11.8 的补充观察**:§11.8 记着陪读「基本不用」。这堵历史墙很可能是原因之一——每次打开都被糊住,谁都不想再点。这和 §11.9 的模板事故是同一类:**入口体验决定功能死活,而九道闸一条都看不见它。**

### 5.18 首页的一句「上次到哪儿了」

#### 为什么

§13.1 已经写下目标:「**第 100 次使用必须比第 1 次更熟悉使用者,而不是每次都像刚开机。**」但首页目前只有事实计数(§5.16),打开时没有任何东西知道你上次在干什么。

这一条是那句目标**最便宜的兑现**:零新表、零新事实,全部由既有的 `material_playback_state`、`segment`、`grammar_encounter` 派生。
...
> 「**你已经三天没学习了**」是评判。
>
> §1.4 禁的一直是后者,从来不是前者。这条线一旦模糊,这个功能就变成打卡。

具体禁止:连续天数、百分比、进度条、「今天该学 X」、任何以缺席为理由的文案、任何推送。

#### 显示规则

**最多一句;取不到就什么都不显示。**到期为空就是空(§1.4),不要用「开始新的一篇吧」这类话填满它。

优先级(取第一个命中的):

1. **有材料中断了** —— `material.status = 'ready'`,且 `position_ms / duration_ms` 落在 **(2%, 80%)** 区间,按 `material_playback_state.updated_at` 取最近一条。
2. **有语法点最近又撞见** —— `grammar_encounter.status = 'encountered'`(撞见过但使用者还没说弄懂)中 `last_evidence_at` 最近的一条。
3. 都没有 → 不显示。

**80% 这个上界来自真实数据,不是拍的。** 2026-08-10 查真实库,4 条播放位置里有 3 条停在 85–88%(那是读完了,提醒它只会烦人),只有 1 条停在 7%(那才是真的中断)。取 90% 会把三条已经读完的材料也变成提醒。下界 2% 排除「刚打开就退出」。

#### 不显示百分比

§4.2 已经写明 `material_playback_state`「**只表达媒体续播状态,不作为学习打卡或游戏化进度**」。把它换算成百分比展示,等于把 §1.4 禁掉的进度条换个写法重新做一遍。**比例只作内部筛选,不出现在界面上。**

改为说位置本身:

- **阅读材料**说「第 N 句」,由 `segment.start_ms <= position_ms` 反查最大 `idx`;
- **视频**说时间 `m:ss`。
...
**点这一句要直接回到那个材料的那个位置**(语法点那一句进语法点详情)。这是它唯一的产出——把你带回上次停下的地方。

因此它也有一条可证伪的判据:**如果它从不被点,它就是装饰**,按 §14.6 同样的态度删掉,不因为「首页看起来更聪明」留着它。

#### 接口

`GET /home/resume`,返回 `{"hint": <对象> | null}`。

用信封而不是裸返回 `null`,是因为**顶层 JSON null 在 Swift 里解不进 Optional**;这一层不是设计上的层次,只是客户端解码的现实。返回的是结构化字段而**不是拼好的句子**——句子是界面文案,归 §1.5 管;放在服务端的是「哪一条值得说」这条选择规则。

- **选择规则只放在服务端一处**,客户端只渲染——与 §5.15 把角度文案留在服务端是同一个理由:判据有且只有一个地方定义。
- 首页已经并行发 4 个计数请求(§5.16),这条并入同一批。
- **失败时整行不显示、不报错**,与计数同一策略:它是一个能用的启动器上的附加信息,不能因为它拿不到就让首页显得坏了。
...
| 2026-08-10 | **放宽语法证据登记口径(§12.5 修订,新增 §11.11)。A1 的原始理由不成立,目标位置因此换了。** 原提案是「治 §11.9 的证据饿死(29 次提问 0 条)」;查真实库发现**那个饿死是模板 bug,2026-08-09 已修好**——当前 `structure` 角度稳定出 2 个点,而 `meaning`/`chinese` 出 0 是正确的(§12.5:查词不登记语法)。真实缺口是另外两处:①**陪读口径已经和入口不符**:原文写「只标注使用者本轮**明确询问**的语法点」,但 §5.15 用可点角度取代预填模板后,使用者点「结构」时根本不点名任何点,按字面无从落地;②**纠错侧有真实漏标**:语法类 5 条挂上 3 条,可确诊的一条是「ないですが」→「お肉は**入っていません**が」,理由讲的就是て形+いる的否定而目录里 `verb-te-iru` 存在。**改动**:陪读口径改为「本轮回答**实质讲解过**、且属于所问那句话的点」,**四条硬边界一字未动**;纠错侧 `category=grammar` 时加一步目录检查、门槛不降;两个提示词版本升为 `companion-turn-v2` / `chat-turn-v2`。**不越 §13.2**:写入只调用 `mark_grammar_encounter(status='encountered')`,自动证据按 §5.10 不得降级手动状态,所以放宽后登记的仍然只是「已撞见」。**实测密度 2 → 2–3**,新增点(`particle-wo`、`verb-te-iru`、`verb-te`、`giving-receiving`)都是真讲过的。**但必须如实记两个负面结果**:①**纠错侧未被证实修好**——重跑「ないですが」时模型改判为 `word_choice`,新检查只在 grammar 时触发,而 word_choice 留空按 §12.5 是正确的,说明那条漏标有一部分是**分类**问题;②**放宽引入一类新错标,且提示词治不了**——「友達に…てもらう」的 に 是动作来源,目录里只有「に(时间)」「に(存在・到达)」,模型挑了最近的一个;加了守卫、连例子本身都写进提示词,跑三次仍 **2/3 错标、1/3 正确省略**,根因是**目录缺项**。带着已知错标上线的前提是 `POST /grammar/evidence/{event_id}/reject` 已存在、错标可撤销且撤销保留为新事实;**不现在补目录**,因为触发它的是实施者造的验证句而非使用者真实撞见,按 §12 原则等真实撞见并撤销过一次再补(§11.11)。**验收**:`ruff` 通过;全新持久库 `harvest_a1_clean` 连跑两次各 **232 项全通过**;新增 4 项测试(含把「被动出现不算证据」钉住);一条既有测试因版本号变化而更新——它钉着写进 trace 的 `prompt_version` 字面值,**版本一升就该改它**,故保留字面值不改成引用常量。iOS 本轮零改动,未重跑构建。材料:`evidence/grammar-evidence-scope/00-gate-a.md` |
| 2026-08-10 | **按第一晚真实使用的三条反馈改 UI(§1.5、§5.15、§5.17 更新)。** ①**陪读弹层刷出一堵墙**:打开时加载**整篇材料**全部历史(上限 40),而模型只取最近 12 条——那一屏既不是使用者要的也不是模型要的。判断是**记住和显示是两件事,此前被绑在一起**:记录必须留(语法证据挂在 `companion_message.id`,§4.3 的删除触发器保证「来源行消失→事件消失」,删了会抹掉骨架来源),显示完全不必。改为 `?segment_id=` 按句过滤,默认只显示当前句,同句以前问过的仍显示(那是真上下文),整篇收进「看这篇以前问过的」入口。**顺带让 §5.17 那条判据真正成立**:上限 40 只是封顶,不是与历史长度无关。**并补一条对 §11.8 的观察**:陪读「基本不用」很可能一部分就是这堵墙造成的,和 §11.9 同类——入口体验决定功能死活,而九道闸一条都看不见。②**角度文案**「意思/自然吗/结构/和中文」被评为「这都是些啥」——那是分类名不是人话。改为口语化,只动 `label_zh`,`question_zh` 与 `focus_zh` 一字未改,故 `LENS_PROMPT_VERSION` 不升。**§11.9 的约束两次决定了最终文案**:第一版「这么说怪吗/跟中文差在哪」排不下一行,横向滚动把最后一个**切成「跟中文差在」**,读起来像错字且正好藏掉「点它得到什么」;改 2×2 解决可读性但吃掉弹层高度;最终按使用者提议**缩短到 15 字**排成一行,滚动作为放大字号时的兜底。③**首页曾长出四种视觉形式**,且**两行裸文字同形不同义**(一个继续学习、一个是 §14 的私人入口)——两次都选「最低调的形式」撞在了一起。重做为单一排版语言(全衬线文字、无卡片无底色块),§14 入口用分隔线 + 右对齐 + 无 chevron 明确分出;并**补上 §1.5 早就要求却一处未实现的时间连续性**:错开淡入 + 轻微上移(280–340ms、间隔 50ms、不弹跳不缩放),计数与「上次到哪儿了」到达时淡入;导航栏大标题改衬线,否则它是屏幕上唯一不属于这套排版的东西。§1.5 同时记下「可爱进入口味轴但兴奋语气仍出局」与「同形必须同义」两条判据。**一个只有真机才抓到的回归**:`segment_id` 过滤的第一版用 `:segment_id IS NULL`,传 None 时 Postgres 推不出参数类型,`GET /companion/{id}` **直接 500**,而全量测试当时是绿的——**不带 segment_id 的路径没有集成测试覆盖**,那正是所有既有调用者走的路。已加 CAST 修复并补测试,且**去掉 CAST 反跑确认它真的会红**(`AmbiguousParameter: could not determine data type of parameter $2`)。**验收**:`ruff` 通过;全新持久库 `harvest_ux_clean` 连跑两次各 **237 项全通过**;新增 3 项测试;iOS `BUILD SUCCEEDED` 与 `TEST SUCCEEDED`;**在模拟器上连真实服务逐项确认**——首页一屏一种形式、点「上次读到第 6 句」直达 しいカフェ 第 6/9 句 0:33、弹层只剩当前句 + 一行入口、四个角度一行排下且完整 |
| 2026-08-10 | **新增 §15 手机上导入并拆分长视频,并据此修订 §3.2。** 起因是使用者一句话:「我发现 Mac 端的后台我不太经常用。」§3.2 原本写着「**不要在 iPhone 上做材料管理界面**」,而这条有两个问题:①**它早就和实现不一致**——iOS 的 `+` 里一直有本地视频导入(§5.2.1);②**理由被真实使用推翻**——「手机上传别扭」成立的前提是「反正你会去 Mac 前面」,前提没了,约束就从保护变成阻碍(与 §1.4 换轴同理)。判据改为「**这一步需不需要键盘和文件系统**」:贴长文、批量管理仍在 Mac 更顺;而从相册挑一个视频、看着画面切成几节,恰恰手机更顺——**片子本来就在手机上,Mac 拿不到它**。**形状**:选文件后**立刻进拆分页,不等上传**(用 `AVPlayer` 播手机本地原文件——§3.3 已实测手机经 Tailscale 播 Mac 视频播不动,且原片编码 iPhone 未必能解),上传在后台并行;「存成合集」提交打点数组;Worker 按点逐节转码为 720p HLS + 纯音频 HLS 全部留本地,每节 `status='downloaded'`,原片转完即删;转录仍按需,走 §5.2 步骤⑤起的既有链路。**两个设计判断值得记**:①**不做静音检测**——初稿是「均分 + `silencedetect` 吸附」,使用者提出直接手机打点更好,砍掉了服务端分析 job、候选过滤、吸附算法、「对齐到停顿」动作与异步到达处理;理由是静音检测唯一的目的是解决「均分时看不见内容」,而**人边看边听边打点本来就知道句子在哪结束——人在环里,启发式就不必要**;②**切分与转码合成一步**,因此切点精确到帧:`-c copy` 只能切在关键帧上(间隔 2–10 秒,可能刚好切掉半句),而 §3.4 已写明**本地上传本来就要转码一次**,所以直接从 `[start,end)` 转出每节 HLS **不增加任何成本**。**合集是分组不是 material**(原片无分句、不可消费,做成 material 会让 §4.1 出现例外):新增 `material_collection` 小表 + `material` 三列,**刻意不存聚合状态**(几节已转录、总时长从 material 派生,存下来只会不一致)。**删除从零建**:`DELETE /materials` 与 `delete_material()` 此前完全不存在(2026-08-10 写测试时只能用裸 SQL 删),数据库侧已经干净(五个外键全 `ON DELETE CASCADE` + §4.3 触发器收敛 `learning_event`),只需额外处理两处——**先删 OSS 前缀对象(失败则整体失败),再删本地文件,最后删行**;反过来会留下无主云对象且再也查不到属于谁。**本次仅设计,代码未实施** |
| 2026-08-11 | **§15 落地完成:手机端拆分页与合集页。** 素材库 `+` 菜单加「长视频，拆开学」;拆分页用 `AVPlayer` 播**手机上那个原文件**、边看边打点(播放中「在这里切一刀」、暂停后 ±0.5 秒与逐帧、时间轴竖线可拖、每节「并到上一节」而不是「删掉切点」),上传在后台跑、顶部一条细线,「存成合集」前的确认**明确写出原片会被删除**;合集页每节一个「转录」按钮、显示实际产出时长与「原片 X 起」、可删单节或整个合集,未转录的节照样能看。文件选择器同时允许 `.movie` 与 `.folder`,`SplitSource` 按内容(`#EXTM3U`)找播放列表,文件夹用 `NSFileCoordinator` 的 `.forUploading` 打成一个 zip 上传。**在模拟器上连真实服务走通全程**:选文件 → 上传完成 → 切两刀得 3 节 → 确认 → Mac 上 `split_video` job `done`,三节 `downloaded`、时长 6651/8311/15107ms、`source_offset_ms` 0/6628/14916,缩略图各取自本节画面。**三处只有真机才暴露的问题,已修**:①**合集里的节又在素材库里重复出现一遍**,分组等于白做——`list_materials` 与 `count_materials` 默认排除 `collection_id IS NOT NULL`;②合集时长显示「0 分」(30 秒被整除成 0),改为按量级选单位;③内部的 `pending-<uuid>` 泄漏到卡片上当来源,改为手机把真实文件名作为 `source_name` 上报。**另修一个我没点进去所以没暴露的 bug**:`GET /collections/{id}` 返回的是不带聚合的裸行,而 iOS 模型要求 `section_count` 等三个字段,**一打开合集页就会解码失败**——现在详情与列表共用同一段 `_COLLECTION_SELECT`,并加测试钉住「两者形状必须一致」;这条是靠看真实响应发现的,不是靠绿的测试。**验收**:`ruff` 通过;全新持久库 `harvest_col2` 连跑两次各 **246 项全通过**;iOS `BUILD SUCCEEDED` |
| 2026-08-11 | **修 HLS 下载包上传:`/videos/uploads` 从来没接受过 `.zip`。** 使用者真机上传文件夹后收到「暂不支持这种视频格式」——**格式从来不是问题**,是后缀白名单里没有 `.zip`(客户端把 content-type 映射成了视频,但文件名仍以 `.zip` 结尾)。**改动**:①白名单加 `.zip`,并在**上传时就解压**而不是留到后台 job——坏包要在使用者还停在上传页时失败,不要在后台 job 里失败;解压逐个成员 resolve 后核对必须落在目标目录内(zip 是攻击者可控输入,`../` 会写到外面);解压后按内容找播放列表,`upload_id` 变成「解压目录/播放列表」的相对路径,因此 `POST /collections` 的校验从「父目录必须相等」放宽为「必须在 uploads 之下」。②**单独传 `play.m3u8` 现在给有用的提示**而不是「不支持这种格式」:「这是播放列表,单独传它没用——请选它所在的整个文件夹,分片要一起上传」;播放列表脱离分片本来就没用,所以这条要说清该怎么做。③`split_video` 完成后对 HLS 源**删整个解压目录**,只删那个播放列表会留下几百个分片永远不走。**又修一个第一次真实拆分就暴露的缺陷**:`create_thumbnail` 的回退分支**没带 `-f hls -allowed_extensions ALL`**,于是 HLS 源第一次取帧失败后彻底拿不到封面——第一次 HLS 拆分里第 3 节就是这么没有封面的;现在前缀算一次、两个分支共用,并加测试钉住。**实测**:单独传 `.m3u8` 得到那句提示;传 zip 得到 `upload_id=pending-<uuid>/play`(那个 `play` 连扩展名都没有);切两刀得 3 节各 10023/10023/10047ms、全部 `downloaded`、**三节封面都在**;uploads 目录残留 0 项,解压目录已清。**验收**:`ruff` 通过;全新持久库 `harvest_zip` 连跑两次各 **247 项全通过** |
| 2026-08-11 | **HLS 下载包终于能在手机上预览了(§15.10 更新):`AVPlayer` 不接受 `file://` 上的 HLS。** 使用者传完文件夹后的实况是「**无法播放,所以我也没办法切分**」——包没问题,是 AVFoundation 对 HLS 只走 HTTP。新增 `LocalMediaServer`(`NWListener`,支持 Range,因为一拖进度条就会来 Range 请求)把那**一个目录**喂给 `AVPlayer`;**只绑回环、端口系统分配、解析结果必须落在该目录内**,`..` 与绝对路径都拒绝。生命周期:离开页面 `teardown()`、`deinit` 兜底、进程死亡是下限,并**实测离开拆分页后 `lsof` 上那个 LISTEN 立即消失**。**这个诊断我做砸了:此前连着三次靠猜去修**(拷进容器、改列表后缀、统一分片后缀),三次全没中,而真实原因就写在本项目自己的 `HLSAssetDownloader` 注释里——**症状在客户端时,先读自己已有的那段代码。****顺带修一个只有上手才会发现的交互缺陷**:时间轴原来只能点、不能拖,而 0:00 时播放头正落在系统返回手势热区里,「按住播放头往右拖」这个最自然的动作**会把页面划走、连切点一起丢掉**;改成时间轴自己用一个 `DragGesture(minimumDistance: 0)` 同时接管点与拖,既能拖着找准位置,也把触摸从返回手势那里抢回来。**实测**(模拟器,两种真实形态各走一遍):①「无后缀列表 + `.ts` 分片」读到时长、画面渲出、`>>` 可跳;②「列表与分片全无后缀」读到 0:30、画面渲出、**从最左边起拖 → 走到 0:10 且页面没被划走 → 切一刀得 2 节**。**一处自证的判断错误**:第一版 fixture 我拿了 `hls-a`(那是**纯音频**包,78KB/6s),看到 AVKit 画出 QuickTime 占位图就以为渲染坏了——占位图对纯音频资源恰恰是正确表现;换 `hls-v` 后画面正常。另记:**模拟器截图抓不到视频层**(mp4 同样是黑的),要用 `simctl io recordVideo` 录下来再取帧才看得见。**验收**:iOS `BUILD SUCCEEDED`;本轮无服务端改动,未重跑套件 |
| 2026-08-11 | **手机流量下改走 OSS 直传(新增 §15.11)。** 使用者反馈「用手机流量的时候上传很慢」——根因与 §3.3 已经测过的 HLS 播放问题同源:手机到家里 Mac 经 Tailscale 这条链路,方向反过来一样弱。判据不用 Wi-Fi/蜂窝(一台手机可以连着别人的 Wi-Fi 却离 Mac 很远,也可以蜂窝网络却直连成功),改为**实测**:上传前先给新端点 `PUT /videos/uploads/probe` 发 256KB 计时,≥600KB/s 才走原来的直传,否则改走签发好的 OSS 直传:`POST /videos/oss-upload-url` 拿一个指向 `temporary/raw-uploads/<uuid>` 的预签名 URL(复用已有的 1 天生命周期规则),手机直接 `PUT` 给 OSS 跳过 Mac,再 `POST /videos/uploads/from-oss` 让新 job `fetch_video_upload` 用 Mac 自己的下行把对象拉回本地解压——产出的 `upload_id` 与直传完全同形,`POST /collections` 分不出走的是哪条路。`_unpack_hls_bundle` 顺带从 `main.py` 挪进 `video.py`(`unpack_hls_bundle` + `HLSBundleError`),因为这次它要被 `worker.py` 复用,和 §15.10 一样是内容判断而非扩展名。**一个真凿的坑**:预签名 URL 不给 `Content-Type`,而 OSS 经典签名把请求实际携带的该头计入验证——`curl --data-binary` 默认带 `application/x-www-form-urlencoded`,拿着没签这个头的 URL 一 PUT 就是 `SignatureDoesNotMatch`,补一个空 `Content-Type:` 头才通;iOS 客户端本就不设这个头,坑只出现在拿 curl 验证时。大小限制从"流式写盘时拦截"挪到"下载前 `head_object` 先查",超限直接删对象不下载。**实测(真实 OSS)**:签 URL → PUT 20KB 测试包 200 → 通知 → job 4 秒内 `done`、解压出 `play` 与分片;拿同一个 `oss_key` 再通知一次,job 如预期 `failed` 报 `NoSuchKey`——证明成功后临时对象真的被删了。**验收**:`ruff` 通过;`pytest` 223 passed, 42 skipped(新增 storage/video/worker/main 四处共 17 项测试,包含 zip-slip、oversize-reject、retry-then-succeed、retry-exhausted);iOS `BUILD SUCCEEDED`。**未做**:模拟器里没能重新走一遍直传路径的界面截图——首页在这次验证的某次冷启动后完全点不动(重启整个模拟器设备也一样),疑似这次会话的输入注入卡在了那一屏,与本次改动的代码路径无关(直传逻辑本身未变,只重命名了枚举 case);已用真实后端往返验证代替 |
| 2026-08-11 | **记录 Rakuo/JustTalk 调研与知心伙伴设计里悬而未决的提案(新增 §11.13、§11.14,§11.10 补交叉引用)。** 按 §0.1,此前 A2/A3/B1/B2、C2、D1–D4、E2–E4 这批提案只存在于对话里,不写进本文档就不算数——本次只是把它们如实记下来,**代码未实施,尚未决定要不要做**。§11.13 是输入侧(来自 Rakuo):语法点出现记录列表(A2)、讲解弹层结构化头部(A3)、语感网络关系 chip(B1,需套 §12.5 纪律)三条待定,原生字幕优先(B2)使用者当场否决记为已否决。§11.14 是对话对象与伙伴(来自 JustTalk 与知心伙伴设计):对话对象四字段 D1–D4(回答 §11.10 卡住的"角色以什么形态回来",§11.10 已补交叉引用)、伙伴模式切换与批注 E2–E4、三条候选红线(第 3 条明确标记可推翻)、伙伴专属验收闸 D4′;写作页字体纸张破例(C2)使用者当场否决记为已否决;FSRS 与 EPUB/PDF 摄入作为零散项简记,后者是提出者建议不做、使用者未表态。**验收**:纯文档变更,无代码改动,未跑测试套件 |
| 2026-08-11 | **阅读/观看页去掉即时陪读问答,改为「收纳疑问→课后找老师一次性消化」(新增 §16,§5.4/§5.15/§11.8 补交叉引用)。** 起因是使用者觉得"AI 聊天的页面太多了",提议阅读页的陪读只留点词查询,另做一个收纳不懂的词/语法/句子的功能,读完一课后去聊天老师那里逐条学习归档。**这不是重新修一次陪读**:§11.9 修好过一次(角度化,3 次提问 2 条证据 vs 修复前 29 次 0 条),但那次修复之后 §11.8「移入待开发」的结论从未被重新评估——这次说明即使问得顺了,「读的时候被打断去问」这件事本身仍是摩擦,§11.9"入口体验决定功能死活"这次用在了更早一步。**三条已确认的取舍**:收纳统一为"一段原文+可选备注"不分词/语法/句子类型;归档只是打勾,不接入 §12 语法骨架;不保留即时问答的退路,查词解决不了的问题(如"为什么用は不用が")一律收纳后课后问。**范围**:按理解写成阅读页与观看页(`ReaderView`/`VideoLearningView`)一起退场,因为两边触发的是同一套 `CompanionSheet`/`CompanionRequest` 机制,只砍一边会留下不一致状态——**但这是本次的理解,不是使用者原话确认过的范围**,实施前要核对。**要退场的**:`CompanionView.swift`、`CompanionSheet.swift`、两个视图里的 `onAsk` 路径、后端 `POST /companion`(失去唯一调用方后是死代码);§5.4/§5.15 保留作历史记录不删,标注已被取代。**不受影响的**:`POST /ask` 独立提问入口(`AskView`,§5.16)、`companion_message` 表、四个角度(lens)系统整套——`AskView` 是独立实现,不复用 `CompanionView` 的界面代码,和这次退场的"材料内即时打断式提问"是两回事。**新数据结构**:`reading_question` 表(`material_id` CASCADE、`segment_id` SET NULL,字段只有原文片段+备注+状态,不分类型);`chat_session` 新增可空 `material_id`(SET NULL)让"选这一课"成为开会话的另一种方式。与 §11.14 提案 E3(`learner_note`)故意不合并:E3 是不挂具体词句的自由心得且不会被"处理完",这次是挂在具体词句上的待办式疑问、会被归档消失。**验收**:纯文档变更,无代码改动,未跑测试套件 |
| 2026-08-11 | **修复删除未转录的拆分视频材料/合集时本地文件永久孤儿化的缺陷(§15.7 补记)。** 根因:`_split_video`/`_transcode_video` 转码完成后只调用 `mark_material_downloaded`,从未把 `hls-video`/`hls-audio`/`asr-audio.m4a` 的本地路径登记进 `media_asset`——这一步原来要等真正上传到 OSS 时才发生,而 §15.6 允许一节视频无限期停在 `downloaded` 不转录,意味着转录之前删除的材料/合集本地字节永远找不回来,不是边界情况,是拆分长视频默认路径下的常态。此修复起初委托给一个后台会话,中途撞上用量限制被打断(卡在写测试这一步,核心改动本身是对的),本人接手完成。**改动**:新增 `Repository.store_downloaded_video_assets()`,转码完成、`mark_material_downloaded` 之前就登记这几个本地路径(`oss_key` 留空);顺带发现并补上一个同源的次生缺陷——真正上传时 `store_video_assets()` 的删旧插新会把这些行连同 asr_audio 一起删掉却不重新登记 asr_audio,导致上传完成的瞬间这个文件又变得找不到,现已在 `store_video_assets()` 里一并重新登记。**测试**:`test_pipeline_state.py` 新增 `_split_video` 专属测试(此前完全没有覆盖)、强化了两个既有 transcode 测试的断言、新增 upload 测试对 `asr_audio_path` 的断言,并给 fake repository/video processor 补上新方法。**实测(真实服务)**:切一个 2 节合集(均停在 `downloaded`)→ 删除整个合集 → 两节共约 1MB 的 `hls-video`/`hls-audio`/`asr-audio.m4a` 确认从磁盘消失,只剩两个空目录壳(既有的无害小瑕疵,与本次修复无关,手动清理)。**验收**:`ruff` 通过;`pytest` 224 passed, 42 skipped |
| 2026-08-11 | **§16 落地完成(后端 + iOS),并修正一处偏离规格。** 后端 231 项单测通过、集成套件连跑两次各 43 项通过;iOS 走完金线。**偏离**:点词的「收纳」按钮最初放在查词弹层正文底部,排在释义/例句/「加入生词表」之后,弹层按 medium 高度打开要滚约两屏才够得着,而 §16 流程第 1 条要求的是「一键」;更硬的是它只在查词**成功**的分支里渲染,查词失败时正文区只剩「重试」,而查不到的词恰恰最该问老师。改为挂进弹层工具栏(挂载点在三个状态分支之外)。**漏检原因**:上一轮验收只走了点句那条路,点词那条一次没走,而两条路共用后端就默认验一条等于验两条——金线必须覆盖设计写明的每个入口。使用者随后追加两条:两个动作都改成工具栏图标不带文字;疑问清单入口改常驻(否则全部归档后「已归档」不可达) |
| 2026-08-12 | **首页独立提问退场,聊天成为唯一问答入口(新增 §17,§5.16 标记退场)。** 使用者原话「首页那个独立提问我也不想要了,没用,我们优化好聊天老师这一个入口我觉得更稳定更好一些」。§16 当时明确保留了 `POST /ask`,理由是它和材料内的即时提问「是两回事」——那个理由只论证了两者不同,没论证独立提问值得单独占一个入口,使用者用下来的判断是不值得。判据与 §11.8/§16 一致:不是功能对不对,是**入口该不该存在**。**退场**:`AskView.swift`、`askCard`、`HomeDestination.ask`、`GET/POST /ask`、`GET /companion/lenses`,整文件 `app/companion.py` 与 `app/lenses.py`(四个角度)及其测试,以及随之失去调用方的若干 repository 方法(含 §16 时就已成孤儿、当时漏扫的 `segment_context`)。**保留不删**:`companion_message`(86 行)与 `companion_grammar_evidence`(9 行)两张表及数据、触发器、以及由它派生 `learning_event` 的回填查询——那 9 条正撑着 **8 个语法点**的来源,§4.3 要求骨架可追溯,删表会让这 8 个点的来源凭空消失。两张表转为**只读历史**,退场的是入口不是记忆(同 §16 对 §5.4/§5.15 的处理)。**首页**:「聊天」从三行列表提到头条位,沿用原 `askCard` 版式,文案「今天想聊点什么？/ 用日语说说话,随手纠错」,不显示话题计数(头条是邀请不是仪表盘)。**连带**:`backup.sh` 会备份 `companion_message`(含 12 行 `material_id IS NULL`),而 schema.sql 里该列仍是 `NOT NULL`、与迁移 0005 不同步——本节退场后这个修复**依然必须做**,否则备份恢复进新建库会在那 12 行上失败 |
| 2026-08-12 | **聊天四项改动(新增 §18,四条使用者反馈)。** ①回答气泡加中文翻译开关:按需生成、按 `chat_message.id` 落库缓存,默认关闭、设置里可改默认;只翻译老师的回答,使用者自己的话服务端 422 拒绝。②纠错讲解改双语:发现 `summary_zh`/`reason_zh` 两个字段名带 `zh` 实际存的是日语(13 条里 12 条、16 条里 15 条)——提示词写着「用中文讲解」但模型一直没照做,产出质量不错所以没人发现字段名和数据对不上。原列改名 `_ja`(纠正标签,不是搬数据),新增真正的 `_zh` 列,卡片可折叠,日文默认展开、中文默认折叠。③修复顶部区域:`configureWithTransparentBackground()` 被同时套到内容滚动后的 `standardAppearance`,导致滚上来的内容跟标题糊在一起,阅读页/观看页同样受影响(§16 验收截图里已有此问题,当时没认出来)。④回答套路:56 条真实消息平均 51 字、高度集中在 2 句,是「固定共情开头+固定套话结尾」的模式;改为按内容浮动长度(上限 5 句)并点名禁止这套节奏,不动 `reply_ja`/`follow_up_ja` 拆分与不连续追问规则(§5.6 治的是「问太频」,这次治的是「答太薄」,是两件事) |
| 2026-08-12 | **新增 §0.4:以后决策只记决策与关键理由,不写验证过程。** 使用者原话「一些没必要的内容或者说检证的过程就没必要往里面写了,主要就记一些我们的主要决策就行吧」。判据:去掉某句话之后读者会不会问"凭什么这么说"——会问就是推翻了不直观假设的关键数据,留;不会问就是操作记录(跑了什么命令、建了几个临时库),删。**只对之后新写的内容生效,§18 及更早的既有内容不回头精简** |
...
| 2026-08-12 | **新增 §19:引入线条插画系统,长期目标贯穿全应用。** 使用者反馈"整体都很简陋,留不住学习欲望",想往 Claude/Anthropic 那种线条插画的视觉表现力方向走。画法规范参照第三方核验仓库([HalfAI1102/anthropic-art](https://github.com/HalfAI1102/anthropic-art),对着 Anthropic 官方 SVG 素材与 CSS 逐条核实过,不是肉眼临摹):满版强调色背景 + 象牙白不规则承载形 + 近黑`#141413`手绘线条的三层结构。背后真实动机是这次改版最终想支持多用户、留住"别人",与 §1.4「不做多用户」正面冲突,但**这次讨论明确搁置**,只记因果不改 §1.4。动效方向定为线条自绘揭示、只播一次不循环,呼应 §1.5 既有的"动效解释状态来去,不做装饰"原则。本节纯讨论,无代码改动,后续先定风格再谈落地页面与批量出图流程 |
...
**2026-08-11 更新,本条结论到此结束**:§11.9 的模板缺陷修好并验证有效之后,这条「移入待开发」的状态从未被重新评估过。使用者最终给出的不是「重新投入」而是「换掉」——即时问答式的陪读整体退场,由 §16 的「收纳疑问 + 课后找聊天老师一次性消化」取代。不再是待处理事项。
...
**为什么现在不改**:改这一段要连带决定角色以什么形态回来(候选方案是给聊天会话一个「对话对象」以定义语体档,顺带治 §11.7),而那个设计尚未定稿——**完整候选方案(D1–D4)见 §11.14**。**先把误导记在这里,避免下一个读者再踩一次**;定稿时与 §13.3–13.6 正文一并改写。参考:§14.5 已就此更正过一次记录。

### 11.11 目录缺项会让模型「挑最近的一个」,而提示词治不了(2026-08-10,已修复)

**现象**:§12.5 放宽陪读登记口径后(A1),对「友達に本を貸してもらいました」提问「结构」,模型把「友達**に**」标成了 `particle-ni-place`。但目录里 に 只有两条——`particle-ni-time`(时间)与 `particle-ni-place`(存在・到达)——而这里的 に 是**てもらう 的动作来源**,两条都不是它。**这是真错标**,正是 §12.5 警告的「悄悄污染骨架」。

**提示词治不了,这是实测出来的,不是推测。** 加了一条明确守卫,连这个例子本身都写进了提示词(「目录里没有精确对应的点时不要退而选最接近的那个,直接不标」),然后跑三次同一句:**2 次仍然错标,1 次正确省略**。约 1/3 生效。

**根因是目录缺项,不是模型不听话。** 模型正确讲解了「友達に」,需要一个 key,而目录里没有对应的点;放宽口径之前它标的点少,这个缺口没暴露出来。

**为什么不现在就补目录**:补一条 `particle-ni-agent` 只是往 `grammar_catalogue.py` 加一行、不需要迁移(§12.2)。但触发它的是**实施者为了验证造的句子,不是使用者真实撞见的内容**,而 §12 的原则是「按你实际撞见的内容组织」。一旦开始按模型「想标什么」补目录,就会滑向按教材补全,那正是 §12.1 要防的。

**已修复(2026-08-10 当日)。** 原处置是「等使用者真的撞见并撤销过一次再补目录」,使用者当日要求直接处理掉,于是改为立即补,**但补的依据换成了一条比测试句硬得多的理由**:

> 目录里**已经有的三个点结构上都需要它**——`giving-receiving`(友達**に**あげる/友達**に**もらう)、`verb-passive`(先生**に**褒められた)、`verb-causative`(子供**に**食べさせる),而目录里只有「に(时间)」「に(存在・到达)」。

所以这不是「因为模型伸手要一个不存在的 key 就补」,而是**补上既有点必需的那个助词**。新增 `particle-ni-agent`「に(相手・動作主)」「给谁／被谁／让谁」,插在另两个 に 之间(条目按 key 幂等 upsert 且 `sort_order` 每次重算,不必追加到末尾);目录从 67 点变为 68 点(§12.2 已同步)。

**实测:补目录 3/3 正确,提示词守卫 1/3。** 同一句跑三次全部标为 `particle-ni-agent`;另外用受身与使役各验一次,分别得到 `particle-ni-agent + verb-passive` 与 `particle-ni-agent + verb-causative`,与上面的理由完全对上。**结论:根因在数据,提示词治不了它。**

**保留的部分**:那条「目录里没有精确对应时不要挑最接近的」守卫仍然留着——目录永远不完整(N3 以上都不在),这条在一般情况下仍然正确,只是不能指望它顶住一个具体缺口。撤销路径 `POST /grammar/evidence/{event_id}/reject` 也照旧,它不是这次的兜底而是长期机制。

**扩充目录的判据已写进 §12.2 和 `grammar_catalogue.py` 的模块文档字符串**,并由 `test_grammar.py` 把「有这三个构造就必须有这个助词」钉成不变量——如果以后有人删掉助词却留着构造,同一类错标会原样回来。

### 11.12 输出侧是否更适合 Mac 键盘——量过之后反而更可疑(2026-08-10)

**起因**:调研 對白/JustTalk(§10 当日记录)时注意到它是**网页优先、键盘优先**的(按 Tab 触发分析、鼠标推到右边缘出聊天)。而 §3.2 的原则是「摄入在 Mac、消费在 iPhone」,理由是粘贴长文、上传视频天然发生在电脑前。**同一条逻辑看起来也适用于输出**:短的口语式来回在手机上没问题,但「写一段真正的表达再被逐句纠错」在 iPhone 上打日文是很痛的——而 §1.3 现在把 AI 聊天老师整个放在 iPhone。

**量了真实数据之后,这条反而站不太住。** 使用者自己写的日语(`chat_message` 里 `role='user'`,38 条):
...
| 最长 | **34 字** |
| **超过 40 字的** | **0 条** |
| 分布 | <10 字 18 条、10–19 字 10 条、20–39 字 10 条 |

**两种读法,这批数据分不开:**

1. 使用者的输出天然就是短句 → 手机完全够用 → 本条前提不成立;
2. 手机压住了长度 → 「从没写超过 40 字」正是输入方式造成的 → 本条成立。

**但有一个能稍微分辨的信号,而它指向第 1 种**:§14 的倾诉入口是**同一部手机上打的中文**,母语、无输入法阻力,5 条平均 **11 字**、最长 24 字——**比日语还短**。如果手机是瓶颈,母语中文应该明显更长。它没有。

**保留的疑点**:两者任务形状不同(倾诉是两分钟内的连续短交换),且中日文的每字信息密度不可直接比,所以这只是倾向性证据,不是结论。

**当前判断:不改架构,也不排进任何计划。** 真正能settle它的观察只有一个——**使用者是否有过「想写长但放弃了」的时刻**。这件事数据看不出来,只有他自己知道。若真的出现过,再考虑把输出侧搬到 Mac 网页;在那之前,按 §11.5 的教训,不为一个量过之后更可疑的假设动 §1.3 与 §3.2。

### 11.13 输入侧待定提案(来自 Rakuo 调研,2026-08-11)

**背景**:调研 Rakuo(§10 当日记录)后提出了一批"能否直接拿来用"的提案。其中语法证据登记口径那一条已经按 A1 落地(§12.5,§10 当日记录)。**按 §0.1,以下几条此前只存在于对话里,写在这里才算数**——都还没有决定要不要做,不是待办清单,是留着等下一次一起过一遍的候选项。

**A2:语法点的"出现记录"列表页,带跳回原材料对应位置。** Rakuo 的语法点详情页会显示"1次出现 / 首次遇到 2026-07-27",下面是来源卡片(标题 + 当时真正撞见的那一句原文),点了能跳回视频对应时间点或文本对应位置。我们 §12.2 已经存了首次/最近来源字段,缺的只是一个消费这些字段的界面。评估:**高,纯增益**,不违反 §13.2"被动看到不等于理解"——这里显示的是撞见记录,不是理解状态的判定。

**A3:陪读讲解弹层加固定结构化头部。** 原句(标出语法点的部分)→ 词 chips → 语法点 chips(带等级徽章)→ 讲解正文,这样一个固定槽位结构。评估:中,主要价值是给 A1 的证据提取提供一个机器可读的锚点,不是界面好看与否的问题。

**B1:语感网络——给语法点配一排带类型的邻居 chip。** 每个邻居标"关系类型(相似/对比)+ 一个词的区分轴",比如「～まま」旁边配「相似/状态持续」「对比/负面持续」的邻居点。评估:中,**但必须先套上 §12.5 那套"宁缺勿滥"的纪律**——这类关系判断错一次,骨架上就多一条误导性的连线,比空着更糟。真做之前要想清楚谁来产出这些关系(模型现场判还是编好的静态表)、错了怎么撤销。

**B2(原生字幕优先,省 ASR 调用)——2026-08-11 已否决,不做。** 使用者当场拒绝,不需要理由展开;记在这里只是为了不让下一次讨论重新把它捡起来当新提案。

### 11.14 对话对象与伙伴:待定提案(来自 JustTalk 调研与知心伙伴设计,2026-08-11)

**背景**:这一批提案互相牵连,建议一起决定,不要拆开单条实施。**按 §0.1,同样只存在于对话里,写在这里才算数**,尚未定稿。

**C2(写作/输出页为字体与纸张破例)——2026-08-11 已否决,不做。** 使用者当场拒绝,记在这里同样只是为了不被重新捡起。

**对话对象(D1–D4,一组,回答 §11.10 卡住的"角色以什么形态回来")**:

| 编号 | 内容 |
|---|---|
| D1 | 会话从"按主题"改为**按对话对象**,四个字段:`counterpart`(对谁)/ `relation`(关系)/ `setting`(场合)/ `register`(语体档) |
| D2 | `chat_correction_item` 必须同时记下当时的 `register`——这是 D1 的**必要条件**,不记的话过去的纠错会失去判据、变成假事实,不是可选项 |
| D3 | "角度"(问什么,理解侧,§5.15 的四个可点角度,无角色)与"对象"(对谁说,输出侧,有角色)是两件不同的事,不合并 |
| D4 | 验收闸:同一句配三个不同对话对象跑纠错,**结论不分叉就整套提案丢掉**(可否证) |

D1 与 §5.6 现有的 16 个精选主题如何共存尚未定案——候选是"主题(聊什么)× 对象(对谁说)"两个独立维度都保留,而不是互相取代,但这会让创建会话多一步,需要真的定下来才知道值不值。

**伙伴(E2–E4,建立在 §14 私人倾诉与 E1"上次到哪儿了"已落地的基础上)**:

| 编号 | 内容 |
|---|---|
| E2 | 伙伴模式 / 老师模式显式切换,伙伴模式下不纠错、不登记语法证据。**与 D1 共用同一处会话模型,需要合并成一个设计**——候选做法是"老师模式才带 `counterpart` 四字段,伙伴模式没有对象"(伙伴不纠错,`register` 判据用不上) |
| E3 | `learner_note`:使用者自己写的一句话批注,只在下次相关时原样引回,不加工、不评判。需要一张新表。这是"知心"设计里被认为最核心的一条机制,但优先级排在 D1/E2 之后,因为它依赖伙伴模式先能被进入 |
| E4 | 给伙伴一个署名,不给性格标语 |

**伙伴的三条候选红线(尚未确认,#3 明确标记为可推翻)**:①不说没有证据的话;②不把自己说过的话当成使用者的记忆;③伙伴可以在使用者打开时"在",不该在使用者没打开时主动找他——**这一条是提出者的判断,不是使用者已经认可的决定**,如果想要一点主动,可行的候选形状是"由内容事件触发"(那篇读到一半的材料朗读音频做好了),不是"由时间触发"(三天没见你了)。

**D4′(伙伴专属的验收闸)**:两周后翻伙伴模式的对话记录,如果没有一句是使用者不会对着纠错机器说的,这个功能就该删掉。

**零散、优先级明显更低、未与其他提案绑定**:

- **FSRS 换掉 §5.9 现在的复习排程**——只是提过名字,没有具体设计,不构成一个可以拿去实施的提案。
- **EPUB/PDF 摄入**——**建议不做**:§4.1 的地基是"材料 = 一段音频 + 一串带时间戳的分句",纯文本电子书不满足这个前提,要做等于动地基。这是提出者的建议,使用者尚未表态。
...
首批目录为 N5–N4 共 67 点(助词 15、动词变形 18、形容词 8、句型 26),覆盖使用者已实测踩到的全部类型。追加 N3 及以上只是往 `grammar_catalogue.py` 里增加条目,不需要改代码或迁移数据。**2026-08-10 追加 1 条 `particle-ni-agent`,现为 68 点(助词 16)**,理由见 §10 当日记录。

**扩充目录的判据(2026-08-10 定,来自 §11.11 那次实测)**:只有两种情况可以加——①**目录里已有的点结构上必需它**(`giving-receiving` / `verb-passive` / `verb-causative` 三个点都需要「动作方 に」,而目录里只有「に(时间)」「に(存在・到达)」);②**使用者真的在上面撞见过**。**不可以因为模型伸手去要一个不存在的 key 就补**——按模型「想标什么」补目录,会一路滑向按教材补全,那正是 §12.1 要防的东西。条目按 key 幂等 upsert 且 `sort_order` 每次重算,所以新条目可以插在它该在的位置,不必追加到末尾。
...
- 陪读(§5.4)把**本轮回答实质讲解过、且属于使用者所问那句话**的语法点写入 `companion_grammar_evidence`;当前材料里只是被动出现的点、助手自行扩展到别处的点、以及使用者其实在问词义或句意的情况都不登记。
  **2026-08-10 修订(原文为「只把用户本轮明确询问的语法点」)。** 改的理由不是想多收证据,而是原口径**已经和入口不符**:§5.15 用可点的角度取代了预填模板之后,使用者点「结构」时**根本不点名任何语法点**,讲什么由模型决定——按字面执行「只登记他明确询问的点」在角度流程下无从落地。登记的仍然只是「已撞见」,永远不会变成「已弄懂」(写入路径只调用 `mark_grammar_encounter(status='encountered')`,而自动证据按 §5.10 不得降级手动状态),因此 §13.2「被动看到不等于理解」的边界没有被动。`COMPANION_PROMPT_VERSION` 升为 `companion-turn-v2`。
  **实测**:同一角度提问从历史基线的 2 个点提高到 **2–3 个**,新增的点(`particle-wo`、`verb-te-iru`、`verb-te`、`giving-receiving`)都是回答里真讲过的。**但它同时暴露了一个目录缺项并因此产生错标,见 §11.11——那个问题不是提示词能修的。**
...
**补记(2026-08-12):`build_journal_messages` 完全没告诉模型现在几点。** 使用者反馈"任何聊天的回答都没有基于时间的依据"——真实数据坐实了这一点:5 条记录集中写于 8月10日 21:30-21:32,第 6 条是 38 小时后(8月12日)写的,而 §14.2 的"记得还没解决的事"就是把这几条原样接成一段历史丢给模型,中间没有任何时间信息,模型自然当成还在同一晚接话。改法:`build_journal_messages` 新增可选 `now` 参数,当历史非空时,在新消息前插入一条系统级提示,给出与上一条的相对间隔(刚才/N分钟前/昨天/前天/N天前/N周前/N个月前)与当前绝对时间,并明确要求不要原样念出来。提示词本身(`JOURNAL_SYSTEM_PROMPT`)未改,但送给模型的消息列表变了,故 `JOURNAL_PROMPT_VERSION` 升到 `journal-v2`。**用真实历史只读跑了一遍生产代码路径**(不新建任何记录),抓到一处措辞错误——"距离上一条已经过了 29 分钟前"里"过了"与"前"重复表达"过去",读起来不通——改成"上一条是 29 分钟前写的"的句式后确认通顺。
...
**起因是使用者 2026-08-10 的一句话:「我发现 Mac 端的后台我不太经常用。」**

§3.2 的原则是「摄入在 Mac,消费在 iPhone」,并写着「**不要在 iPhone 上做材料管理界面**。手机上粘贴长文、上传视频文件都很别扭」。这条判断有两个问题:

1. **它已经和实现不一致。** iOS 素材库右上角的 `+` 早就有本地视频导入(§5.2.1 的 `LocalVideoImportView`,系统文件选择器 + 流式上传),文档却还写着不要做。
2. **它的理由被真实使用推翻了。**「手机上传别扭」成立的前提是「反正你会去 Mac 前面」。使用者实际不常开那个后台,于是这条约束从保护变成了阻碍——和 §1.4 当初换轴是同一种情况:约束画的线,和真实行为不在一处。

**因此修订 §3.2**:摄入不再限定在 Mac。**判据换成「这一步需不需要键盘和文件系统」**——贴长文、批量管理仍然在 Mac 更顺;而**从相册/文件里挑一个视频、看着画面切成几节**,恰恰是手机比 Mac 更顺的事(片子本来就在手机上)。

### 15.2 形状:手机打点,服务端按点切

```
① iPhone:选一个本地视频文件
② iPhone:立刻进拆分页 —— 用 AVPlayer 播**手机上那个原文件**,边看边打点
   ↑ 不等上传。上传在后台并行跑,一小时的片子上传要几分钟,那几分钟你在切
③ iPhone:点「存成合集」 —— 提交打点数组(毫秒)
④ Worker(split_video):按点逐节从原片转码为 720p HLS + 纯音频 HLS,全部留在本地
⑤ 每节 material.status = downloaded(待转录),零 OSS 上传、零云流量
⑥ 原片转完即删
⑦ 使用者点某一节的「转录」→ 走 §5.2 步骤⑤起的既有链路(上传 OSS → ASR → 翻译)
```

**为什么预览播的是手机本地文件,不是 Mac 上的原片。** §3.3 已经实测过「手机用蜂窝流量经 Tailscale 播 Mac 上的视频基本播不动」;而且原片是原始编码,iPhone 未必能直接解。本地文件则秒开、可拖、可逐帧,还不花流量。

### 15.3 不做静音检测,因为人已经在看了

设计初稿是「均分 + ffmpeg `silencedetect` 把切点吸附到停顿处」。**使用者提出直接在手机上打点、把时间传给服务端就行——这个更好,并且砍掉了一整套东西**:服务端静音分析 job、几百个候选点的过滤、吸附算法、「对齐到停顿」动作、以及建议点异步到达时不能覆盖手动点的处理。

理由是:静音检测存在的唯一目的是解决「均分时看不见内容、会切在句子中间」。**使用者边看边听边打点,他本来就知道句子在哪结束**——再让服务端猜一遍是多余的。**人在环里,启发式就不必要。**

### 15.4 切分与转码合成一步

**结论**:每一节直接从原片的 `[start, end)` 转出 HLS,切分与转码是同一步。

- **不增加任何成本** —— §3.4 已经写着「只有本地上传、编码不兼容或直接封装失败时才转码」,手机上传的视频本来就要转码一次;
- 原片的唯一用途就此用完,所以切完即删(§15.7)。

#### 一处必须更正的理由(2026-08-11 实测)

**本节初稿写的理由是错的。** 原文说「`-c copy` 只能切在关键帧上,典型间隔 2–10 秒,于是在 5:04.3 打的点会被挪到最近的关键帧、可能切掉半句」。实测推翻了它:

造一段 30 秒测试片,关键帧**强制只在 0/10/20 秒**(`-g 150 -keyint_min 150 -sc_threshold 0`),每帧画面上打出当前秒数,然后在 **13.5 秒**切:

```
-ss 13.5 -i src -c copy   →  Duration: 00:00:16.50   首帧画面显示 13s
```

**恰好 16.50 秒,精确的。** 现代 ffmpeg(实测 7.1)在输入侧 `-ss` 配 `-c copy` 时会保留解码所需的前置帧并重写时间戳,不会把起点吐回关键帧。**所以「会滑动几秒」这个担心对当前的 ffmpeg 不成立。**
...
```
pts_time:0   iskey:0   type:B
```

而 **HLS 要求每个分片从关键帧开始**(§3.4 的封装还开了 `independent_segments`)。一个开放 GOP 起头的片段没法直接拿去封装 HLS——要么重新编码,要么重打关键帧,而后者就是转码。

**所以决定不变,理由换成这一条。** 代码里 `create_hls` 在带时间窗时强制关掉 `direct_video_copy`,守的是这个,不是那个已经不成立的关键帧偏移。

#### 另一处实测到的边界

HLS 分片是 6 秒粒度,因此**每节的实际时长会被分片边界量化**:上面那次 13.5→20.0 秒(应为 6.50)实际得到 `Duration: 6.60, start: 1.400000`。这不影响学习(每节自己的时间轴从 0 开始,ASR 也按该节音频独立跑),但**界面上显示的时长要用实际产物的时长,不要用打点算出来的差值**——两者会差零点几秒。

### 15.10 源可以是一个 HLS 下载包,而不只是一个视频文件(2026-08-11)

**起因**:使用者从网页下载的视频常常不是一个文件,而是**一个播放列表加上几百个分片**(截图里是 `play` 与 `TZhO-00000…`,`.m3u8`/`.ts` 只是被 iOS 隐藏了)。而 iOS 的 `.fileImporter` 限定 `[.movie]` 时**根本不会显示播放列表**,单独挑分片又毫无意义——这正是「导入不方便」的一部分。

**不需要先合并成 mp4**:ffmpeg 可以直接读本地播放列表,`[start, end)` 也能照常切。但实测踩了三个坑,三条都必须写下来:

| 坑 | 现象 | 解法 |
|---|---|---|
| 列表没有 `.m3u8` 后缀 | `Not detecting m3u8/hls with non standard extension` | 强制 `-f hls` |
| 分片没有已知后缀 | `Error when loading first segment` | `-allowed_extensions ALL`(ffmpeg 的安全限制) |
| **HLS 的 `-ss` 是按分片定位的** | 切 13.5 秒,时长正确但**画面从 0s 开始** | **`-ss` 放到 `-i` 之后**(输出侧,解码后丢弃) |

**第三条是最要紧的,而且和普通文件正好相反。** 普通 mp4 用输入侧 `-ss` 又快又准;HLS 用输入侧只会定位到「包含该时刻的那个分片的开头」,而真实下载包的分片普遍 4–10 秒——**正好是会切掉半句的量级**。改到输出侧之后,同一次切的首帧画面从 0s 变成正确的 13s。

**判据:按内容识别,不按扩展名。** 读头 7 个字节是不是 `#EXTM3U`(`is_hls_playlist`)。下载器产物有的保留 `.m3u8`/`.ts`、有的全剥掉、iOS 还会把它藏起来——**字节是唯一可靠的信号**。

**实测通过**:用「无扩展名列表 + 无扩展名分片」这种最难的形态,切 13.5→20 秒,产出 3 个视频分片与 3 个音频分片,首帧画面为 `13s`。

**缩略图刻意仍用输入侧定位**:封面差几秒没人在意,而输出侧定位要为一个从 50 分钟处开始的节把前面全解码一遍。**边界要准,封面不必。**

**手机侧的形状**:`.fileImporter` 同时允许 `.movie` 与 `.folder`;选中文件夹时按内容找到播放列表;上传用 `NSFileCoordinator` 的 `.forUploading` 把整个文件夹打成一个 zip(系统自带,不引第三方,也不用发几百个请求),服务端解压后按内容找列表。**zip 对已压缩的视频几乎不减小**,所以上传量与原片相当——不新增成本,但一小时的片子仍要传几分钟,而那几分钟正是你在切的时间。

**预览要在 App 内起一个本机 HTTP 服务(2026-08-11)。** 上面写的「`AVPlayer` 直接播它」是错的,实测不成立:**`AVPlayer` 不接受 `file://` 上的 HLS 播放列表**——包本身没问题(ffmpeg 读得很顺,`AVAssetDownloadURLSession` 的 `.movpkg` 也照播),是 AVFoundation 对 HLS 只走 HTTP。所以「边看边切」只剩一条路:`LocalMediaServer` 用 `NWListener` 在 `127.0.0.1:<系统分配>` 上把那一个目录喂给 `AVPlayer`,并支持 Range(一拖进度条就会来 Range 请求)。

范围刻意收得很窄,因为这是在自己进程里开了一个可被访问的口子:**只绑回环**、端口由系统分配、**只服务那一个目录**且解析结果必须落在目录内(`..` 与绝对路径都拒绝)——不是「把沙盒挂到 HTTP 上」。生命周期三层:离开页面 `teardown()`、`deinit` 兜底、进程死亡是绝对下限;**已实测离开拆分页后 `lsof` 上那个 LISTEN 立即消失**。

**在这之前我连着三次靠猜来修这个播放问题**(拷进容器、改列表后缀、统一分片后缀),三次都没中。真实原因就写在本项目自己的 `HLSAssetDownloader` 注释里。**症状在客户端时,先去读自己已有的那段代码,再动手。**

**还有一条只有真机/模拟器上手才会发现:时间轴原来只能点,不能拖。** 而 0:00 时播放头正落在系统返回手势的热区里,于是「按住播放头往右拖」这个最自然的动作**会把整个页面划走,连切点一起丢掉**。改成时间轴自己用一个 `DragGesture(minimumDistance: 0)` 同时接管点与拖:既能拖着找准位置,也把这个触摸从返回手势那里抢了回来。

### 15.5 合集是分组,不是新的材料抽象

**合集不是 material。** §4.1 的地基是「材料 = 一段音频 + 一串带时间戳的分句」,而原片没有分句、不可消费,把它做成一条 material 会让 §4.1 出现例外。改为一张很小的分组表:

- `material_collection(id, title, created_at, updated_at)`
- `material.collection_id`(可空,`ON DELETE CASCADE`)、`material.collection_index`(第几节)、`material.source_offset_ms`(这一节在原片里的起点,只为界面能说「从 10:21 开始」)

**不在合集上放任何聚合状态**(几节已转录、总时长)——那些都从 `material` 派生,存下来只会不一致。

### 15.6 转录仍然按需,而且这条路本来就通

需求「想学哪节就转录哪节」**不需要新机制**:§5.2 步骤④⑤已经把下载/转码与转录拆成两步,`status='downloaded'` 就是「待转录」,`POST /materials/{id}/start-transcription` 就是那个按钮。合集只是让它一次面对多节。

**没有转录的一节照样能看**,只是没有字幕——不转录不等于不能用。

### 15.7 删除:现在完全不存在,要从零建

`DELETE /materials/{id}` 与 `DELETE /collections/{id}` 都不存在,`repository` 里也没有 `delete_material()`(2026-08-10 写测试时只能用裸 SQL 删)。素材库缺了它只会一直堆。

数据库侧是干净的:`segment`、`companion_message`、`job`、`media_asset`、`material_playback_state` **五个外键全部 `ON DELETE CASCADE`**,§4.3 的删除触发器同时让 `learning_event` 收敛。因此删除只需额外处理数据库管不到的两处:

1. **本地文件** —— 按 `media_asset` 登记的路径删,必须在删行之前取出路径;
2. **OSS 对象** —— 删 `materials/{id}/` 前缀下的全部对象,否则会一直付存储费。

**删除的顺序与失败语义**:先删 OSS(可重试、失败则整个删除失败并保留数据库行),再删本地文件,最后删数据库行。反过来会留下无主的云对象且再也查不到它属于谁。**删一节只删那一节;删合集删掉它的全部节。** 手机上已下载的离线分片由客户端在下次打开素材库时按服务端列表清理。

**这条链路曾经有一个洞,直到 2026-08-11 才补上**:「按 `media_asset` 登记的路径删」这句话本身没错,但 `_split_video`/`_transcode_video` 完成转码后只调用 `mark_material_downloaded`,**从未把 `hls-video`/`hls-audio`/`asr-audio.m4a` 的本地路径写进 `media_asset`**——那一步要等到真正上传到 OSS 时才发生。§15.6 明确允许一节视频无限期停在 `downloaded`(不转录),这意味着**任何在转录之前被删除的材料或合集,本地字节都会永久留在磁盘上找不回来**,不是罕见边界情况,是拆分长视频功能默认路径下的常态。新增 `Repository.store_downloaded_video_assets()`,在转码完成、`mark_material_downloaded` 之前就把这几个本地路径登记进去(`oss_key` 留空,因为还没上传);真正上传时 `store_video_assets()` 会用同样的删旧插新覆盖掉这些行,但原来这一步漏登记了 `asr_audio_path`,导致上传完成的那一刻这个文件又重新变得找不到——一并补上。**实测**:切一个 2 节的合集(都停在 `downloaded`)→ 删除整个合集 → 两节的 `hls-video`/`hls-audio`/`asr-audio.m4a`(共约 1MB)确认从磁盘消失,只剩两个空目录壳(与本次修复无关的既有小瑕疵,手动清理,不影响任何实际数据)。

### 15.8 界面要求

**精度全靠打点,所以时间轴必须能微调。** 播放中点「切一刀」最快;暂停后用 ±0.5 秒与逐帧按钮定准。切点在时间轴上是竖线,可拖。

**列表里每节后面是「并到上一节」,不是「删掉这个切点」**——前者说的是结果,后者要在脑子里换算(与 §1.5 同一种取向)。

**每节显示真实时长**,不显示「约 5 分钟」。

**「存成合集」是不可逆的显式动作**,且**必须在确认里写明原片会被删除**:切错了就得从手机重新传一遍(§15.7 的原片即删是使用者 2026-08-10 的决定)。不做成随手一点。

上传进行中时拆分照常可用,只有「存成合集」要等上传完成;进度是顶部一条细线,不是挡住画面的遮罩。

### 15.9 明确不做

- **不做手机端切分后上传 N 个文件。** 上传一次整片,服务端切——手机切要导出 N 个文件、上传 N 次、还费电,而服务端 `ffmpeg` 那一步本来就要跑。
- **不做静音/场景自动检测**(§15.3)。
- **不做先转录再按句切。** 最准,但要先花 ASR 的钱才知道怎么切,与「默认不转录」直接冲突。
- **不在合集上做进度、完成度或百分比**(§1.4)。
- **不做重切。** 原片已删,重切必须重传;这是使用者选的取舍,不用代码去绕。

### 15.11 手机流量下,直传 Mac 太慢就改走 OSS(2026-08-11)

**起因**:§15.10 把 HLS 下载包的预览做通之后,使用者实测发现「用手机流量的时候上传很慢」。原因不是本机服务本身——§3.3 早就测过 HLS **播放**经 Tailscale 在蜂窝网络下不行,同一条弱链路反方向(上传)一样弱。手机连家里的 Mac,大概率要么被 Tailscale 打到 DERP 中转、要么撞上家庭宽带本就不对称的上行带宽(下行远大于上行是常态)——**这两条都与"是不是 Wi-Fi"无关**,一台手机可以连着别人的 Wi-Fi 但离 Mac很远,也可以连着蜂窝网络却恰好直连成功。

**判据是测出来的,不是猜的**:正式上传前,手机先给 `PUT /videos/uploads/probe` 发 256KB 计时。≥ 600KB/s(约 4.8Mbps)才走原来的直传;探测失败或不够快,改成:

1. `POST /videos/oss-upload-url` 换一个指向 `temporary/raw-uploads/<uuid>.<ext>` 的预签名 `PUT` URL(6 小时有效,给慢速上传留够余量;这个前缀本来就有 1 天的生命周期规则,中途放弃也不留垫脚石);
2. 手机把原始文件直接 `PUT` 到 OSS,完全跳过 Mac 这一跳；
3. `POST /videos/uploads/from-oss` 告诉 Mac 去取;`fetch_video_upload` 这个 job 用 Mac 自己的下行(通常是家庭带宽更强的那一侧)把对象拉回本地、按内容解压 HLS 包(复用 §15.10 的 `unpack_hls_bundle`),产出与直传完全相同形状的 `upload_id`——`POST /collections` 分不出这次走的是哪条路。

**预签名 URL 不带 `Content-Type`,两侧都不带**。OSS 经典签名会把请求实际携带的 `Content-Type` 计入验证,而不只是签名时传的值;`curl --data-binary` 默认附带 `application/x-www-form-urlencoded`,拿着不带该头签出的 URL 一 PUT 就是 `SignatureDoesNotMatch`——**这一步是真栽了一次跟头才确认的**,补一个空 `Content-Type:` 头之后 200。iOS 端的 `putFileToOSS` 本就没有设置这个头,所以这个坑只出现在用 curl 验证时,不会出现在真实客户端里。

**大小限制挪到了下载前**:直传时 `max_video_upload_bytes` 在流式写盘时就会拦下;走 OSS 则要等 `fetch_video_upload` 跑`head_object`先读一次大小,超限直接删掉那个临时对象、不下载——不能等一整个超大文件拉到本机磁盘才发现太大。

**实测(真实 OSS,而不是 mock)**:签发 URL → `PUT` 20KB 测试包(200)→ 通知 → job 在 4 秒内变 `done`,`payload.upload_id` 是`pending-<uuid>/play`,本机确实解压出了 `play` 与分片;再拿同一个 `oss_key` 发第二次通知,job 按预期 `failed`、报 `NoSuchKey`——**证明第一次成功后临时对象真的被删掉了**,不是留着占地方。

**没有做,也不打算做的**:没有用 Wi-Fi/蜂窝这个信号做判据(上面已经说明为什么它测不出真正想测的东西);没有对探测結果做多次采样取平均——一次 256KB、一个阈值,够用就没必要更复杂。

**补记(2026-08-12):使用者报"上传失败 403",排查未定位到根因,先加诊断能力,待真机复现**。逐项复核了这条链路上所有会产生 403 的环节,全部健康:

- 用**真实运行中的服务进程**走一遍完整流程(`POST /videos/oss-upload-url` 拿真签名 → 不带 `Content-Type` 的 `PUT`),200 成功——不是绕开服务直接调 Python 函数,是和手机走的同一条路。
- Mac 系统时钟与 OSS 服务器 `Date` 头相差 1 秒,排除时钟偏差导致的签名过期误判。
- 预签名 URL 里的 `Signature` 参数确认被正确 URL 编码(`%3D` 而非裸 `=`),排除特殊字符被中间层解释成空格那类经典坑。
- 桶 ACL(`public-read`)、Referer 白名单(未设置)、Bucket Policy(未设置)均无额外限制。
- `temporary/raw-uploads/` 下没有任何遗留对象——说明失败发生在签名校验阶段,字节从未落地,不是"上传成功但客户端误报失败"。
...
## 16. 阅读/观看页去掉即时陪读问答,改为「收纳疑问→课后找老师一次性消化」(2026-08-11)

**起因**:使用者反馈「感觉现在做的这个 AI 聊天的页面太多了」,提议阅读页的陪读只保留点词查询,另外做一个把不懂的词、语法、句子收纳起来的功能,读完一课后拿着这些收纳去聊天老师那里逐条学习并归档。

### 这不是重新修一次陪读,是承认它的问题域被换了个更小的入口接管

§11.9 修好过一次陪读:预填模板锁死提问自由度(29 次提问 0 条语法证据),换成角度化点选后验证有效(3 次提问 2 条证据)。但 §11.8「陪读移入待开发」的结论**在角度化修复之后从未被重新评估过**——文档里没有任何「修好之后陪读复活了」的记录。这次的提议说明:即便点一下角度就能问、不用再打字,「读的时候被弹层打断去问一句话」这件事本身仍然是摩擦。§11.9 的教训「入口体验决定功能死活」这次用在了更早一步的地方——不是「问题问得顺不顺」,是「要不要在这一刻问」。

**决定**:阅读页(与观看页,见下方范围说明)不再有即时问答式的陪读弹层。点词只做查释义;点词或点句都可以顺手「收纳」,收纳的内容留到课后,去聊天老师那里一次性过。

### 三条已确认的设计取舍

1. **收纳不分类型**,统一是「一段原文 + 可选备注」。不区分词/语法/句子——读的时候往往并不知道自己卡住的是不是个语法点,让使用者当场分类只会增加摩擦。
2. **归档就是打勾,不接入语法骨架**。聊完一条、用户自己觉得懂了,就把这条标记为已处理,到此为止——不写入 `grammar_encounter`,不生成证据。这条收纳机制是一次性的学习待办,不是长期记忆系统;它和 §12 的语法骨架是两件事,故意不合并。
3. **不保留即时问答的退路**。原来陪读能问「结构」「和中文」这类查词解决不了的问题,现在这类问题只能收纳后课后问——这是有意的取舍,不是遗漏。

### 范围:读和看的即时陪读都退场,不只是阅读页

使用者原话是「阅读页的那个陪读」,但触发即时陪读的是同一套机制(`CompanionSheet` + `CompanionRequest`),观看页(`VideoLearningView.swift`)点字幕单词/整句和阅读页(`ReaderView.swift`)点正文单词/整句走的是完全相同的代码路径。**本设计按两边一起退场来写**,因为只砍一边会留下「读的时候不能问、看的时候还能问」这种不一致状态,而且新机制(收纳 + 课后消化)对两边同样适用。**这是本次的理解,不是使用者原话直接确认过的范围**——实施前应该先核对一句。

### 与「点词查询」的关系:不是新建,是把默认动作换过去

点词查释义(`WordLookupSheet` + `POST /dictionary/lookup`)本来就存在,只是此前只能从剪贴板查词、手动输入或点助手回复里的词进入,阅读正文本身点词走的是陪读而不是它(§5.4)。这次的改动是**把阅读/观看正文的点词行为,从「打开陪读弹层」换成「打开这个已经存在的查词弹层」**——查词逻辑、提示词、接口全部复用,不新建。

点句(原本触发整句提问陪读)不查词——整句查词没有意义。改为直接收纳这句,不弹出任何浮层,只给一个不打断阅读节奏的轻量反馈(比如底部一闪而过的提示),这是零摩擦的关键:§11.9 已经证明过,弹层每多一次点击,后面的使用率就往下掉一截。

### 数据结构

新表,不是塞进 `vocabulary`(粒度只到词、走 Leitner 复习调度)或 `grammar_encounter`(专属语法骨架、要求真实撞见+可撤销):
...
CREATE TABLE IF NOT EXISTS reading_question (
    ...
    material_id   BIGINT NOT NULL REFERENCES material(id) ON DELETE CASCADE,
    segment_id    BIGINT REFERENCES segment(id) ON DELETE SET NULL,
    excerpt       TEXT NOT NULL,   -- 原文片段(词/短语/整句,不分类型)
    note          TEXT,             -- 可选备注,使用者自己写
...
`material_id` 用 `CASCADE`(材料没了,收纳自然没有意义);`segment_id` 用 `SET NULL`(和 `companion_message` 一致的模式——即使句子引用失效,也只丢失跳回原位的能力,不丢整条收纳记录)。

`chat_session` 新增一个可空外键,让「选这一课」成为开新会话的另一种方式,与现有「按主题」并列,互不影响:

```sql
ALTER TABLE chat_session ADD COLUMN IF NOT EXISTS material_id BIGINT REFERENCES material(id) ON DELETE SET NULL;
```

`SET NULL` 而非 `CASCADE`:材料被删不该连带抹掉已经聊过的对话记录,只是这条记录从此失去「属于哪一课」的标签。

### 流程

1. 阅读/观看时点词 → 查词弹层,弹层上带一个「收纳」按钮,一键把这个词存成一条 `reading_question`(`excerpt` 就是这个词,`note` 留空,可选补一句)。
2. 点句 → 直接收纳这一整句,不查词、不弹层,只给一闪而过的确认反馈。
3. 材料页某处(具体位置留给实施阶段设计,候选是材料详情页或阅读页顶部)显示「N 个疑问待处理」,点开是这一课的收纳列表,可以在这里编辑备注或直接删除某一条。
4. 「去问老师」跳转到聊天,新开一个绑定了 `material_id` 的会话,开场由服务端把这一课全部 `pending` 的收纳拼成上下文交给模型(类似「我在读这篇材料时有几处不懂:1. ××(备注:××) 2. ……,请一条一条帮我讲清楚」),模型按顺序讲解。
5. 每讲完一条,**使用者自己**判断懂了没有,手动把对应条目标记为已归档——归档权在使用者手上,不是模型自动判定,这和 §13.2「被动看到不等于理解」的原则一致。

### 明确要退场的东西

`CompanionView.swift`、`CompanionSheet.swift`,以及 `ReaderView.swift`/`VideoLearningView.swift` 里触发它们的 `onAsk` 路径,连同后端 `POST /companion` 端点——这条链路失去唯一的调用方,是死代码,应该删除而不是留着占位。§5.4、§5.15 整段作为设计过程的历史记录保留不删(§0.2 只做局部替换,删除会丢失「为什么当初这么设计、后来为什么变了」的经验),但视为已被本节取代。

**不受影响、明确保留的**:`POST /ask`(§5.16 的独立提问入口 `AskView`)、`companion_message` 表、四个角度(lens)系统整套。`AskView` 是「不依附任何材料,随手粘贴一句话就能问」的独立工具,和这次要退场的是「读一篇材料时的即时打断式提问」完全是两回事,继续保留、不受这次决定影响。`AskView.swift` 是独立实现(不复用 `CompanionView.swift` 的界面代码),两者退场与否可以完全分开处理,互不牵连。

### 与 §11.14 提案 E3 的区别

E3(`learner_note`)是「随手写一句批注,以后相关时原样引回」,不涉及具体某个词/句,更接近一句自由心得。本节的 `reading_question` 是「一条待办式的疑问,附着在具体的词/句上,课后要被消化并划掉」。两者都是「先轻量记下、后消费」的同一种直觉,但消费方式和生命周期不同(E3 不会被"处理完"从而消失,`reading_question` 会),故意做成两张不同的表,不要合并。

### 验收

**2026-08-11 已实施**。后端 231 项单测通过,集成套件在独立持久库上连跑两次各 43 项通过;iOS 在模拟器上走完金线:点句收纳 → 顶栏徽章实时计数 → 疑问清单改备注/归档/取消归档/删除 → 「去问老师」开出绑定 `material_id` 的会话且开场白确实逐条点到收纳内容 → 面板里勾掉一条后清单同步。测试期间在真实服务产生的 `reading_question` 行与聊天会话已全部清理。

**实施中发现并修正的一处偏离规格**:点词的「收纳」按钮最初放在查词弹层正文底部,排在释义、例句、「加入生词表」之后,弹层以 `.medium` 高度打开时要滚约两屏才够得着,且样式是最弱的纯文字链接——本节流程第 1 条要求的是「一键」,实测不是。更硬的问题是它当时只在查词**成功**的分支里渲染,查词失败时正文区只有一个「重试」,而查不到的词恰恰最该问老师。改为放进弹层工具栏(与「完成」并列,图标+「收纳」二字),挂载点在三个状态分支之外,因此加载中/查词失败/查词成功都在;成功路径已在模拟器实测,加载与失败两态由挂载位置保证。

教训与 §11.9「入口体验决定功能死活」同源,但这次是在**同一节内部**重犯:功能写完了、接口通了、测试过了,可使用者点不到就等于没有——验收时只验了点句那条路,点词那条路没走,漏检的正是使用者第一时间反馈的那个点。**金线验证必须覆盖设计里写明的每个入口,不能因为两条路共用后端就只验一条。**

**使用者随后追加的两条(2026-08-11,已实施)**:

1. **「收纳」和「加入生词表」两个动作都只用图标,并排放在查词弹层工具栏**。原来「加入生词表」是正文底部的主按钮、「收纳」是它下面的文字链接,现在两个都是工具栏图标(托盘/书签),不带文字,说明退到 `accessibilityLabel`。使用者原话:「不用这么多字也可以,一个简单的图标都可以」。两个动作故意不合并成一个——生词表是走 Leitner 调度的长期记忆,收纳是一次性待办(见本节取舍 2)。随之去掉的还有「暂无可用释义,无法加入生词表」那句解释,改为按钮直接禁用。
2. **疑问清单入口改为常驻,待处理为 0 时也显示**(空心托盘、不显示计数)。原来按 §1.4「空托盘不配一个常驻图标」只在有待处理时出现,但这样一来**全部归档之后就再也进不去「已归档」分区**,等于把看过的记录锁死。这里 §1.4 让位于可达性:入口本身很轻(一个图标),而藏起来的代价是一整块历史不可达。

**补记(2026-08-12):疑问清单入口改用弹窗,不再挤进阅读页的导航栈。** 使用者反馈"查看收纳内容打开新页面,返回时又要从头播放"——根因是 `ReadingQuestionListView` 一直用 `NavigationLink` 推入阅读页所在的同一个 `NavigationStack`,而 `ReaderView.onDisappear` 会 `player.stop()` 并清空恢复位标记(当初是为退场前的陪读弹层写的,§16 把陪读砍了但这个 `onDisappear` 留了下来,继续套在所有共享同一栈的导航上)。改为 `.sheet`:阅读页/观看页的托盘按钮从 `NavigationLink` 改成本地 `Button` 弹出弹窗,弹窗内套一个独立的 `NavigationStack` 承接"去问老师"到聊天的跳转,退出靠新增的「完成」按钮或下滑手势。`HomeDestination.questions` 因此失去唯一调用方,一并删除(枚举 case 与 `HomeView` 的路由分支)。真机模拟器实测:开弹窗、编辑、点「去问老师」跳进聊天、整体关闭,阅读位置全程停在原处未被重置。观看页(`VideoLearningView`)存在完全相同的代码结构,一并按同样方式修过,不等使用者第二次报告。

## 17. 首页独立提问退场,聊天成为唯一的问答入口(2026-08-12)

**起因**:使用者原话「首页那个独立提问我也不想要了,没用,我们优化好聊天老师这一个入口我觉得更稳定更好一些」。

### 这是 §16 那条判断的第二次应用,不是新判断

§16 砍掉阅读页即时陪读时,明确写过「不受影响、明确保留的:`POST /ask`(§5.16 的独立提问入口 `AskView`)」,理由是它「不依附任何材料,随手粘贴一句话就能问」,和材料内的即时打断式提问是两回事。那个理由本身没错,但它只论证了**两者不是同一件事**,没有论证**独立提问这件事值得单独存在一个入口**。使用者用了一段时间之后给出的判断是后者:不值得。

判据和 §11.8/§16 一致——不是功能做得对不对,是**入口该不该存在**。聊天已经能承载「随手问一句」(它本来就是自由输入 + 纠错),独立提问相对它只多了「四个角度点选」,而角度这套东西在 §11.9 被验证为对**阅读中的即时提问**有效(3 次提问 2 条证据),从来没有单独验证过它对「首页随手问」也是必需的。少一个入口、把力气集中在聊天上,是更稳的结构。

### 范围:入口和代码退场,数据保留

**退场**(失去唯一调用方后即为死代码):

- iOS:`AskView.swift`;`HomeView` 的 `askCard` 与 `HomeDestination.ask`;`APIClient` 的 `companionLenses()`/`askMessages()`/`ask(text:lens:)`;`Models` 的 `QuestionLens`
- 后端:`GET /ask`、`POST /ask`、`AskRequest`、`GET /companion/lenses`
- 后端整文件:`app/companion.py`(提示词组装 + 单轮生成)、`app/lenses.py`(四个角度),连同 `tests/test_companion.py`、`tests/test_lenses.py`
- `repository` 上随之失去调用方的:`add_companion_message`、`delete_companion_message`、`standalone_ask_messages`、`companion_messages`、`record_companion_grammar_evidence`,以及 §16 删除 `POST /companion` 时就已成孤儿、当时漏扫的 `segment_context`

**保留,不删**:

- `companion_message`(86 行)与 `companion_grammar_evidence`(9 行)两张表及其全部数据,以及 `trg_companion_message_learning_event_delete` 触发器
- `apply_baseline` 中由 `companion_grammar_evidence` 派生 `learning_event(kind='companion_question')` 的那段回填查询

理由是 §4.3:这 9 条证据正撑着 **8 个语法点**的「为什么这个点在你的骨架里」。删表会让这 8 个点的来源凭空消失,而骨架的可追溯性是 §12 的立身之本。两张表从此变成**只读的历史**——没有任何代码再写入,但读取路径完整保留。这和 §16 对 §5.4/§5.15 的处理是同一种做法:**退场的是入口,不是记忆**。保留可逆,删除不可逆;将来若确认这段历史无用,再单独清理。

`companion_message.lens` 列里存着那 12 条独立提问当时用的角度 id,列保留(数据),生成角度问句的代码删除。

### 首页:聊天升到头条位

`askCard` 腾出的是首页最显眼的位置(续读提示之下、三行列表之上)。直接留空会让首页开头只剩一列链接,所以把「聊天」从三行列表里提上来,沿用 `askCard` 原来的字号与版式(27pt 衬线标题 + 副标题),文案改为「今天想聊点什么？/ 用日语说说话,随手纠错」。列表剩「素材」「积累」两行,`entrance` 的错开序号相应前移。

这不是给聊天加权重,是**正门仍然是一句招呼、而不是一列目录**——§1.5 的延续。头条位不显示话题计数:头条是邀请,不是仪表盘(§1.4)。

### 与备份/新建库的关系(一处必须注意的连带影响)

`backup.sh` 只排除日记两张表(§14.3),`companion_message` **会被备份**,其中 12 行 `material_id IS NULL`(独立提问留下的)。而 `app/schema.sql` 里 `companion_message.material_id` 至今写的是 `NOT NULL`,与迁移 `0005_standalone_ask_messages.sql`(把该约束 drop 掉)不同步。**本节退场之后这个不同步依然必须修**:新建库时 schema.sql 说了算,把备份恢复进去会在那 12 行上直接失败。已单独开任务处理,与本节独立。

### 验收

**2026-08-12 已实施并验证**。后端 204 项单测通过;集成套件在独立持久库 `harvest_s17` 上连跑两次各 41 项通过(跑完即删)。真实服务重启后:`GET /ask`、`POST /ask`、`GET /companion/lenses` 三个端点均返回 404,而 `/materials`、`/chat/sessions`、`/grammar`、`/vocabulary`、`/home/resume` 全部 200。iOS 构建通过,首页与选定方案一致,头条点进去正常打开聊天。

**保留决策已按执行验证,不只是推理**:服务重启会重跑 `apply_baseline` 的回填,重启后 `companion_message` 仍 86 行(其中 12 行 `material_id IS NULL`)、`companion_grammar_evidence` 仍 9 行、`learning_event(kind='companion_question')` 仍 9 条,覆盖 8 个语法点;直接查 `GET /grammar/particle-wa`,证据条目里那条陪读提问的问题原文、时间与当时的日语上下文都完整返回。**退场入口没有动到记忆。**

**删除牵出的三处集成测试回归(已修,不是绕过)**。它们都拿已退场的 `add_companion_message`/`record_companion_grammar_evidence` 当脚手架,但测的是**仍然存在**的行为,所以逐个改写而不是删掉:

1. 语法投影测试——改为直接向 `companion_message` / `companion_grammar_evidence` 插入 fixture(故意插两次,以此覆盖 `(message_id, point_id)` 唯一约束"一次提问不能算两次撞见");下游的回填、`has_companion_question` 投影、`grammar_evidence()` 的 `companion_question` 类目、以及材料级联删除时清理 `learning_event` 的触发器,全部原样保留——这些正是本节承诺保留的读取路径,理应有覆盖。
2. 决策轨迹测试——`companion_grammar_index` 这个 `call_source` 从此再无写入方,那半段测的是死行为。但它的注释指出自己是 §5.13「静默路径必须留下可归因失败轨迹」目前**唯一**的集成覆盖(该检查 2026-08-09 从已删除的记忆重建路径搬过来一次)。故**再搬一次**到仍然活着的 `chat_correction_index`:monkeypatch 掉 `mark_grammar_encounter` 使其抛错,断言 `failure_stage == "project_grammar"`。语义上更强了一点——这条路径是**咽下**异常的(已保存的纠错不该因为骨架投影失败就显示为失败),所以轨迹是它唯一的现形处。
3. 材料删除级联测试——只需要一条挂在材料上的 `companion_message` 行来证明级联会清掉它,改为直接插入。

**教训**:这三处是「删除一个写入方,却有测试把它当共享脚手架」的典型。单测全绿(204 项)时它们仍然是红的,因为它们全部标了 `integration`、默认被 deselect——**删除类改动必须把集成套件也跑一遍,单测绿不能作为删干净的证据。**

## 18. 聊天老师:翻译开关、双语讲解、顶部区域与回答套路(2026-08-12)

使用者一次提了四条,其中两条是界面缺陷,两条改变教学内容的呈现。四条共用一节,因为它们都只动聊天老师这一个入口(§17 之后它是唯一的问答入口,值得集中打磨)。

### 18.1 回答气泡下面加中文翻译开关

**要求**:助手回答气泡下方一个图标按钮,一键开/关中文翻译,默认关闭;设置里可以改默认是开还是关。

**翻译从哪来:按需生成 + 落库缓存,不随每轮回答一起生成。** 默认是关闭的,若跟着每轮回答一起生成,等于为一个大多数时候不会被看的东西在每轮都付 token。按需的代价是第一次点开要等一下,但只等一次——译文按 `chat_message.id` 落库,第二次点开是即时的,重启应用也还在。

只翻译助手的回答。使用者自己写的日语是他自己的表达,不在这条要求里——服务端也据此拒绝(422),不只靠界面不给按钮。

**开关只用图标不带文字**:它挂在每一条回答下面,写上"中文"两个字会沿着整个对话重复几十遍。说明退到 `accessibilityLabel`。

**18.1 实测**:
- 端点契约:不存在的消息 404;使用者自己的消息 422(`只翻译老师的回答。`);助手消息第一次 `cached=false` 并落库,第二次 `cached=true` 且**文本完全一致**——重复开关不会让译文在两次之间悄悄变样。
- 译文本身:`今日は最近読んだ本について話しましょう。/ この前、何か本を読みましたか。` → `今天我们来聊聊最近读过的书吧。/ 前几天你读了什么书吗？`,原文的分段保留了,没有附带任何语法说明。
- 界面:默认关闭时空心图标、不显示译文;点一下变实心强调色并显示;设置里打开默认之后,进会话即显示、无需点击。测试完把默认改回了关闭。
- **模拟器上单点 SwiftUI Toggle 注入无效,要用横向滑动才翻得动**。这是注入事件的问题不是应用缺陷(真机点按正常),记在这里省得下次当成 bug 再查一遍。

### 18.2 表达调整:日文讲解 + 中文讲解,两种都可折叠

**先说一个发现:现有字段名一直在说谎。** `chat_correction.summary_zh` 与 `chat_correction_item.reason_zh` 的名字和提示词(`Explain briefly in Chinese`)都说是中文,但实测**存的是日语**:13 条 summary 里 12 条是日语,16 条 item reason 里 15 条是日语(按假名出现判定)。模型一直在无视那条指令,而因为产出的日语讲解质量不错、界面也照常显示,这个偏差从来没被发现。使用者这次的原话「保持现在的这种日语讲解内容就挺好的,加一个中文讲解即可吧」正是在描述他实际看到的东西——他要的不是"新增日文讲解",而是"把我现在看到的这个留住,再补一个中文的"。

**这条记下来的价值不在这个字段,而在:提示词写了不等于模型照做,字段名写了不等于里面装的是那个东西。** 只有去看真实数据才知道系统在干什么。§16 那次是「验收只验了一半入口」,这次是「读代码却没读数据」,同一类错误的第三个面。

**决定**:

1. 把现有两列改名为 `summary_ja` / `reason_ja`——里面本来就是日语,这是**纠正标签,不是搬运数据**。
2. 新增 `summary_zh` / `reason_zh` 两列存真正的中文讲解。
3. 迁移要处理那各 1 条真的是中文的历史行:按假名判定,没有假名的搬进 `_zh` 列而不是留在 `_ja` 里。只有两行,但这是使用者自己的学习记录,不该为了省事就给它贴错标签。
4. 提示词改为**两种都要产出**,并明确哪个字段装哪种语言;日文讲解的风格与详略保持现状(使用者认可),中文讲解要求简洁、讲清中日差异。
5. 中文讲解与日文讲解**同轮一起生成**,不像 18.1 的译文那样按需。理由:纠错本来就不是每轮都有(56 条助手消息里只有 16 条 item),而一张纠错卡是一个整体,让其中一半要等第二次请求会把已经不简单的卡片变得更碎。

**界面**:两段都可折叠,**日文讲解默认展开,中文讲解默认折叠**。「原句/更自然」和每条「原文→修正 + 类别」留在折叠区之外——它们两种语言下都一样,而且是你先扫一眼判断这张卡要不要细看的部分。某一语言整段缺失时(迁移前的历史行只有日文)那一段连折叠行都不显示:一个点开之后什么都没有的折叠行比没有这一行更糟。

**18.2 实测**:
- **迁移在真实库的副本上先跑**(用 `TEMPLATE` 克隆,原库不动):13 → 13 行、16 → 16 行一条不丢;12 条日语留在 `summary_ja`、1 条真中文搬进 `summary_zh`;15 条日语留在 `reason_ja`、1 条搬进 `reason_zh`;`_ja` 列里**没有任何缺假名的行**,分类判据成立。搬走的那条肉眼可辨是中文:「将中文菜名"番茄煮馍"转化为日语中自然且易于理解的描述。」再跑一次 `apply_schema` 无待应用迁移、数据不变。之后才应用到真实库,结果一致。
- **新纠错双语齐全**:真实模型跑「最近、勉強に苦手です。本を読むています」,两处错都给出了日文与中文两版讲解,且中文版确实是**为中文母语者写的**而不是逐字翻译——`「苦手」「好き」「嫌い」等表达主观感受的词,其对象必须用「が」提示…这是中文母语者常因直译"对……苦手"而犯的错误`。
- 模拟器上确认:日文段默认展开、中文段默认折叠,两段都能点开点合;历史行(只有日文)不显示中文折叠行。
- 单测 204 通过;集成套件在独立持久库上连跑两次各 41 通过(跑完即删)。测试期间的探测会话与临时库已全部清理。

**一处刻意留下的命名债**:`learning_event` 的 payload 里那份讲解仍然存在键 `reason_zh` 之下,而值现在取自 `reason_ja`。这两个词摆在一起就是错的——但那个 payload 已经写进了 §4.3 视为既成事实的历史行,里面装的正是日语。改键名等于重写事实层;从改名后的列取值则让**内容**与已有的完全一致。用一条注释和这一段把债记下来,而不是拿重写历史去还。同理,payload 组装处加了 `COALESCE(reason_ja, reason_zh)` 兜底:少一种语言不该让整条学习事实丢失。

**实施中踩的一个坑**:迁移文件里注释写了一个百分号(「~92% of the time」),迁移是走 `exec_driver_sql` 执行的,psycopg 把裸百分号当参数占位符,于是 `apply_schema` 整个失败——连带 20 多个毫不相干的集成测试一起红(它们都调用 `apply_schema`)。已在文件里注明"本文件不得出现百分号"。**这也是"删除/迁移类改动必须跑集成套件"的又一次印证**:单测全绿时这个错误完全看不见。

### 18.3 顶部区域:滚动上来的内容会和标题糊在一起

**根因**:`DesignTokens.applyNavigationBarAppearance()` 把 `configureWithTransparentBackground()` 同时套给了 `standardAppearance`、`scrollEdgeAppearance` 和 `compactAppearance` 三种状态。

- `scrollEdgeAppearance`(内容停在顶部)透明是**有意**的,那个没有横线的干净纸面观感靠它;
- `standardAppearance`(内容滚到栏下面)透明就意味着正文直接透上来,和标题、工具栏按钮叠在一起。

已在模拟器复现:聊天里往上滚,橙色的用户气泡正好压在标题「我发现我的日语不能完…」上,最上面还有一条气泡从状态栏后面透出来。

**修法**:`scrollEdgeAppearance` 保持透明,给 `standardAppearance` 和 `compactAppearance` 一个背景,让滚到下面的内容被遮住。**这是全局修复,不只是聊天页**——阅读页、观看页同样一直有这个问题(§16 验收期间的截图里就能看到正文从标题栏后面透出来,当时没认出来那是缺陷)。底部输入区本来就有正确的不透明处理,这次是把顶部补齐,两头一致。

### 18.4 回答别再是固定的一句话

**先量了再改**。56 条真实助手消息:平均 51 字;句数分布以 2 句为主(30 条),3 句 14 条,1 句 10 条;62% 含问句。形状高度一致——`一句共情或建议` +(空行)+ `一句提问？`。可复现的语气习惯:共情用「〜ですね」「〜ですよね」「〜と思います」开头,建议一律以「〜といいですよ」「〜が効果的ですよ」「〜てみてください」收尾。

**两个可动的地方,只有一个能动**:

- **不能动**:`reply_ja` / `follow_up_ja` 的拆分,以及「上一轮已提问则本轮不提问」。这是 §5.6(2026-08-06)实测修出来的:此前 `follow_up_ja` 必填,导致 18 条助手回复 **100% 以问句结尾**,对话变成连续质询,5 个会话里用户平均只发 2.6 条就走。那条规则治的是「问得太频」,现在这条抱怨是「答得太薄」,是两件事,不能借着改后者把前者一起推翻。
- **要动**:`Reply with 1–3 short Japanese sentences.` 这条硬上限——它让每一轮的体量都一样,这才是"每次都是固定的一句简单的话"的直接来源。改为「该短就短,该举例就举例」:允许在建议后面跟一个具体做法或例子。同时**明确禁止上面那套固定节奏**:不要每轮都以共情句开头,不要每次都用同一组句尾收建议,要求换着走(直接反应 / 补一个事实 / 举一个具体例子 / 反问其中一处),但仍然不主动追加问句。

使用者选定的档位是「砸套路 + 允许说得具体一点」,不是「明显展开像真人长篇说话」——后者会把聊天推向"读段落",而 §16 砍掉即时陪读的起因正是「AI 页面太多、读不动」。**这个上限是有意留的。**

提示词版本 `chat-turn-v2` → `chat-turn-v3`。测试里那条钉版本号的断言不是这次的主题(它属于 §12.5 的语法标注测试),但它是个有用的绊线:改了提示词却不动版本号,轨迹里就分不清「模型变差了」和「我们改了措辞」。这次正是它先响的。

**18.4 实测(真实模型,4 轮探测,跑完即删)**:回答字数 58 / 73 / 94 / 156,对比改动前 56 条真实消息的平均 51 字且高度集中在 2 句——长度终于随内容变化了。四轮里那组套话句尾(「〜といいですよ」「〜が効果的ですよ」「〜てみてください」)**全部没有出现**。问「精読と多読はどう違いますか」时(156 字)直接给定义,不再先共情,而且**主动接住了前面几轮里使用者自己说过的困境**(「点数は取れるのに内容が頭に残らないと感じるなら…」)——"承接上一轮"是改动前从来没有出现过的行为。

**没有完全根除的**:「〜ですね」这类共情/夸奖开头仍然出现在第 2、4 轮(输入分别是诉说困难和「毎日30分読んでいます」),但在第 1、3 轮没有出现。判断是**可以接受**而不是没修好:原来的问题是「不管说什么都先共情一句」,现在它看语境了;对着"我每天读30分钟"回一句肯定,是人也会这么说。若之后仍觉得腻,下一步应该收紧的是"夸奖"这一类,而不是再压长度。

### 验收

四条全部实施并验证,分别记在各小节里(18.1/18.2 的实测段、18.3 的复现与修复、18.4 的前后对比数据)。后端 204 项单测通过;集成套件在独立持久库上连跑两次各 41 项通过(跑完即删)。测试期间在真实服务产生的探测会话、纠错行与临时数据库已全部清理,`chat_correction` 回到 13 行。

**这一节反复撞到同一件事**:18.2 的字段名说谎、18.3 的透明导航栏、18.4 的固定套路,都是**代码读起来没问题、跑起来或量一量才知道不对**的类型。18.2 最明显:读代码得出的结论(讲解是中文)和数据说的(12/13 是日语)正好相反。加上 §16 那次「只验了一半入口」,「先量再改、改完再验」这条在这一节里被证明了四次。

## 19. 视觉插画系统:引入线条插画,长期目标贯穿全应用(2026-08-12)

**起因**:使用者反馈"整体都很简陋,留不住学习欲望",希望调性往更有视觉表现力的方向走,明确参照对象是 Claude/Anthropic 的审美,并特别提到官网上"线条人物和物品的设计"以及有时出现的动效。

**背后的真实动机,已知但暂不展开**:使用者说得很直接——这次改版最终是为了"也让别人能用这个 app 并留下来",目前的单用户状态只是暂时的。这与 §1.4「不做多用户、不做发布上架」正面冲突,涉及账号体系、数据隔离、多租户后端架构,以及 §14 私人倾诉功能"只有你自己能看"这条隐私前提的重新设计——工作量与本节的视觉改版完全不是一个量级。**本次讨论已明确把这个问题搁置**,§1.4 暂不修改,不作为本节的一部分实施。留在这里是为了这条因果链不丢:如果以后重新讨论视觉方向,不该忘了它最初的驱动力其实是产品定位的转变,不只是审美偏好。

### 目标不是"特效",是一套具体的插画语法

讨论中先排除了一个误读:§1.5 已经写着「接近 Claude.ai 那种克制、温暖、以内容为中心的设计感」,使用者说喜欢的"Claude 的审美"容易被当成同一件事,但实际是两套不同的视觉语言——claude.ai 网页本身安静克制,官网新闻页和宣传物料用的是另一套**线条插画系统**。查了 claude.com、anthropic.com、App Store 的 Claude 应用页面,均未见到线条插画,确认这套语言出现在官网新闻文章配图与社媒物料上,不是产品界面本身。

**画法规范来自第三方核验,不是肉眼猜的**:参考了 [HalfAI1102/anthropic-art](https://github.com/HalfAI1102/anthropic-art) 这个仓库——它对着 Anthropic Newsroom 的页面 HTML、CMS 记录、官方 SVG 素材(Hand House、Object Globe)和 CSS 逐条核验过,区分了"验证过的实现细节"和"视觉推断"两部分,比对着一两张截图临摹准得多。该仓库与 Anthropic 无隶属关系,是独立研究整理的公开规范。

**三层结构**:
1. 满版不透明强调色背景,覆盖画面四角,不留白边或透明区域
2. 一块不规则**象牙白**(`#FAF9F5`)承载形,占画面约 55%–80%
3. 最上层是**近黑色**(`#141413`,不是纯黑)的手绘线条与实心圆点,允许在承载形边缘处小幅"溢出"到背景色上

**核验过的强调色**(每张只用一个色系,不做彩虹拼盘):cactus 淡绿 `#BCD1CA`、heather 淡紫灰 `#CBCADB`、oat 燕麦 `#E3DACC`、clay 陶土橙 `#D97757`(与 Harvest 现有 accent 色`#B8654A`同系,天然契合)、olive 橄榄绿 `#788C5D`、sky 天空蓝 `#6A9BCC`、fig 无花果粉 `#C46686`、coral 珊瑚粉 `#EBCECE`。

**线条要求**:粗细不均、圆头、带手抖感的连续曲线;造型天真简化、故意不完全对称;绝对排除摄影感、3D、渐变、阴影、"精致企业矢量感"、Logo、文字。"手"是反复出现的核心意象(官方两个素材都是手在托/框/指/握着核心物体),题材上可以延展到 Harvest 自己的语境(书、茶、日式元素),但要素克制在一个清楚的隐喻上,不堆砌。

### 范围与顺序

**长期目标是贯穿整个应用**,但第一步只做**风格定稿**,不铺到具体页面——风格没锁定就铺开,容易做成前后不一致的四不像。已经用 `mcp__visualize` 手绘验证过三层结构本身(满版陶土色背景 + 象牙白承载形 + 手托书的近黑线条),确认规范可执行,但**这一步仍然是示意稿,不是可以直接用的素材,也没有写入任何代码**。

**产能问题,已经预见但未解决**:手写 SVG 路径坐标画一张示意图可以,但支撑不起"贯穿整个应用"需要的数量(几十张不同场景的插画)。风格锁定后的现实路径是用图像生成工具按本节的规范批量出图,再转成矢量资源接入 App——不是继续手绘。这一步具体怎么做,留待风格定稿后再谈。

### 动效:线条自绘揭示,只播一次

使用者提到官网有时"还能动"。讨论确定的方向是**线条自绘动画**——笔触从起点画到终点,像被手实时画出来,而不是循环播放的背景动效。技术上用 SwiftUI 原生的 `.trim(from:to:)` 配 `withAnimation` 即可,不需要额外的库或素材,已用 HTML/CSS 的 `stroke-dashoffset` 做过一版可交互演示确认节奏可行。

**播放时机是唯一被明确定下来的规则**:只在插画**第一次出现**时播放一次,此后停留在画完的静态状态,不循环、不每次进入页面重播。理由是与 §1.5 已有的动效原则保持一致——「动效用于解释状态从哪里来、到哪里去,不用作无意义装饰」,自绘揭示解释的是"这张图刚刚出现在你眼前",重复播放就从"解释状态"滑向"看着好玩的装饰",而后者正是这条原则要拦住的东西。

### 验收

无——本节是纯设计讨论的记录,`mcp__visualize` 产出的示意图不是可用素材,代码未改动。下一步(风格定稿方式、具体先落地到哪些页面、批量出图的实际流程)留待后续讨论确定。
