...
本节是 §13.9 M1 里「为记忆提取、投影、召回与删除定义统一决策记录」那一条的精确契约,也是 M1 通过条件中「任一投影异常能凭决策元数据定位到来源入口、规则版本与失败阶段」的落地。范围只覆盖**已经存在的后台增强路径**,不为将来的圆桌、主动建议或模型调用预先设计字段。
...
- 定义 1 位主持人和首批 3 位参与者:自然表达/日本人听感、语法与信息结构、中文母语迁移视角;
- 每个角色有稳定名字、背景、语气、专长、不擅长、发言条件与不可越过的教学边界;
- 每个角色维护可版本化的角色设定集,区分共同且受保护的教学内核、不可漂移的核心身份区、可演进的关系/偏好区;核心区变更必须随代码审查和角色回归样本发布,不能由运行时反思自行改写;
- 定义各角色可读的学习事实范围,不向每个角色注入全部历史;
- 角色之间不共享可变消息数组;每次参与只获得任务说明、白名单证据与允许的关系摘要,完成后返回结构化结果。任何角色输出都没有写入学习者事实、长期记忆或直接对用户发送消息的权限;
- 角色的关系状态只允许影响表达通道,不能进入日语事实判断、纠错准确性、服务意愿或功能可用性决策;
- 用同一批真实句子对照测试角色稳定性、观点差异与语法事实一致性。
