...
**手机侧的形状**:`.fileImporter` 同时允许 `.movie` 与 `.folder`;选中文件夹时按内容找到播放列表,`AVPlayer` 直接播它做预览;上传用 `NSFileCoordinator` 的 `.forUploading` 把整个文件夹打成一个 zip(系统自带,不引第三方,也不用发几百个请求),服务端解压后按内容找列表。**zip 对已压缩的视频几乎不减小**,所以上传量与原片相当——不新增成本,但一小时的片子仍要传几分钟,而那几分钟正是你在切的时间。
