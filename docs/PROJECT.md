...
- **下载进行中不切换播放源**:连续前缀会随下载逐片增长,若以「本地已有分片」为判据,会在下载途中把正在播放的在线播放器换成只有几片的离线播放器——画面黑屏、被换下的在线播放器因无人暂停而继续出声、字幕停在新播放器的位置、错误显示为 Cannot Open。因此仅在该素材没有正在进行的下载时才使用离线源;切换时暂停被换下的播放器并携带播放位置
