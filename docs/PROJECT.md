...
本节受 Alice 的「越用越懂你」、分层记忆、活人感、自进化与圆桌脑暴启发,但只吸收适合日语学习的产品与工程原则,不以复制通用桌面 Agent 为目标。参考:`https://alice.miyang.cn` 与 `https://github.com/itshen/Alice_methodology`。
...
通过条件:至少四类现有学习行为可以投影到同一套事件契约;重放两次不重复;删除原证据后相关索引、记忆与投影按设计收敛。
...
- 区分共同且受保护的教学内核、角色的稳定设定与可演进的关系/偏好区;
