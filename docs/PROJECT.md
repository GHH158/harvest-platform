...
### 11.11 目录缺项会让模型「挑最近的一个」,而提示词治不了(2026-08-10)
...
**因此当前处置**:
- 保留放宽后的口径与那条守卫(它至少有 1/3 的时候管用,且不影响正确标注的点);
- **依靠既有的撤销路径**:`POST /grammar/evidence/{event_id}/reject` 已经存在(§12.5),错标可由使用者撤销,且撤销本身作为新事实保留、不是编辑历史。**这是这条能带着已知错标上线的前提**;
- **等使用者自己真的在 てもらう / 使役 / 被动这类需要「动作来源 に」的句子上撞见并撤销过一次错标,再补目录条目。** 那时补的依据是真实撞见,不是我的测试句。
