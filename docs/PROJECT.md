...
## 19. 探索过线条插画方向,看到真实渲染效果后搁置(2026-08-12)

使用者反馈"整体都很简陋",一度想往 Claude/Anthropic 官网新闻页那种**线条插画**风格走(满版强调色背景 + 象牙白不规则承载形 + 近黑手绘线条的三层结构,参照第三方核验仓库 [HalfAI1102/anthropic-art](https://github.com/HalfAI1102/anthropic-art)),也讨论过"贯穿全应用"的动效方向(线条自绘揭示、只播一次)。

**用 `mcp__visualize` 画出几版真实渲染的示意图、放进实际页面比例之后,使用者认为不好看,当场决定搁置**——判断依据是看到的实际效果,不是抽象讨论。首页视觉调性这个话题没有结束,后续单独再谈,只是不朝这个具体方向走了。

副产品留了下来,与插画风格本身无关:验证出一条"写 SVG → 用 macOS 自带 `qlmanage` 转 PNG → 读回来自查"的回路,不需要额外依赖(`rsvg-convert`/`cairosvg`/`inkscape` 都不用装),下次做需要眼见为实的可视化工作时可以直接用。

讨论中一并浮出一个更大的动机(使用者最终想支持多用户、不只是单用户自用)仍然待定,不因本节搁置而改变——§1.4「不做多用户」暂未修改,留在这里只是不让这条因果线丢掉。
