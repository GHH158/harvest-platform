...
- [ ] 用同一批至少 10 个真实问题完成去署名对照,检查可辨识度、增量、事实一致性与冗余度。

通过条件:去掉角色署名后仍能从关注角度与表达方式辨认差异;不为了制造性格而在日语事实上互相矛盾。

当前状态:M2-A 的代码与自动契约测试完成后,仍不能仅凭 prompt 不同宣布 M2 通过;最后一项需要真实模型输出和人工盲测。未过此闸前不进入 M3 圆桌实现。

首轮真实调用已经完成但验收无效:角色输出高度同质,且 trace 表缺失、日志泄露角色映射、匿名结果未持久化。问题与修正见 `docs/reviews/M2-role-blind-evaluation.md`;当前已转入 `role-perspective-v2` 与 `m2-blind-v2` 复验准备,仍须另行授权 30 次外部调用。M2 复选框保持未完成。
