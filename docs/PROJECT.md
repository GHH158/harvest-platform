...
`ROLE_PERSPECTIVE_PROMPT_VERSION=role-perspective-v1`。结构化输出固定为:
...
**方法论审计、M1 与 M2-A 精确契约/安全预览已完成。当前只允许执行 M2 的 10 题去署名真实模型盲测并据结果调校角色;盲测通过前不得进入 M3 圆桌,也不得扩建角色阵容、向量设施或主动推送。** M3–M7 继续只保留产品方向与验收边界,按真实使用证据逐步开启。
