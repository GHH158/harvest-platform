...
```text
..                                                                       [100%]
2 passed, 26 deselected in 0.43s
```
...
iOS 测试目标为 iPhone 17 Pro Simulator（iOS 26.5）；部署目标为 iOS 18.0，App 与 `HarvestTests` 均实际构建，3 个测试均实际执行。
...
| P5 | 本地视频分块上传和限制；`transcode → upload_video → asr_video → translate_video → ready` 状态机；双语字幕页面 | 未验证真实 ffmpeg 视频、OSS、ASR、翻译；视频链接摄入、真正的跟读音频模式和视频离线下载尚未完成 |
